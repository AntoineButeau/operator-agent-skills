# Behavioral OS Pack Example

## Input context
A team is working through How Companies Actually Behave: Operating Principles, Decision Rules, and the Hidden System Behind Culture and needs an agent to convert messy operating context into a usable artifact. The agent receives the named owner, current source-of-truth links, recent signals, deadlines, constraints, and the next operating-review date.

## Pack route
1. Start with `skills/revealed-behavior-audit.md` when the problem is still ambiguous.
2. Move through the remaining skills only when their distinct job is needed: Decision Rule Extractor, Culture Operating Gap Review, Behavior Change Lever Map.
...
## Agent prompt
Use `packs/26-company-behavior/skills/revealed-behavior-audit.md` from the Behavioral OS Pack. Inspect the context for revealed behavior, decision rule, culture gap, behavior lever, produce the requested output schema, mark assumptions, and recommend the smallest next move.

## Expected output shape
- Concrete use case and decision owner
- Evidence table with facts, assumptions, gaps, and source-of-truth links
- Domain diagnosis using pack terms such as revealed behavior, decision rule, culture gap, behavior lever
