# Cross-Functional Issue Room

## Use when
Use this workflow when you are working on how to be a great coo: the operating system behind the company and need to handle the room step without overlapping the other skills in the pack. Its distinct job inside the COO Operating Model Pack is to produce the cross-functional issue room; use the neighboring skills for Company Operating Model Map, Execution Risk Register, COO Operating Dashboard.
...
4. Separate this skill's job from the rest of the pack: deliver the cross-functional issue room, and list what should be routed to Company Operating Model Map, Execution Risk Register, COO Operating Dashboard.
...
## Output
Return a cross-functional issue room with this schema:
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready cross-functional issue room tied to operating model.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Company Operating Model Map, Execution Risk Register, COO Operating Dashboard, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Cross-Functional Issue Room workflow for the following How to Be a Great COO: The Operating System Behind the Company situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the cross-functional issue room using the output schema, mark assumptions, and list the human review gates before anyone acts.
