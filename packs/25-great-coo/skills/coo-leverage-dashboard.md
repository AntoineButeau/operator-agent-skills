# COO Operating Dashboard

## Use when
Use this workflow when you are working on how to be a great coo: the operating system behind the company and need to handle the dashboard step without overlapping the other skills in the pack. Its distinct job inside the COO Operating Model Pack is to produce the metric decision brief; use the neighboring skills for Company Operating Model Map, Cross-Functional Issue Room, Execution Risk Register.
...
4. Separate this skill's job from the rest of the pack: deliver the metric decision brief, and list what should be routed to Company Operating Model Map, Cross-Functional Issue Room, Execution Risk Register.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready metric decision brief tied to operating model.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Company Operating Model Map, Cross-Functional Issue Room, Execution Risk Register, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the COO Operating Dashboard workflow for the following How to Be a Great COO: The Operating System Behind the Company situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the metric decision brief using the output schema, mark assumptions, and list the human review gates before anyone acts.
