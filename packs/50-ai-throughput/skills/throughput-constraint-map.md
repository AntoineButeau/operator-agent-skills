# Throughput Constraint Map

## Use when
Use this workflow when you are working on From Productivity to Throughput and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the AI Throughput Pack is to produce the throughput constraint map; use the neighboring skills for Queue Health Review, Agent Leverage Plan, Throughput Metric Design.
...
- Domain signals to inspect: queue, bottleneck, cycle time, review latency, batch size.
...
3. Inspect the domain mechanics for this pack: queue, bottleneck, cycle time, review latency, batch size. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the throughput constraint map, and list what should be routed to Queue Health Review, Agent Leverage Plan, Throughput Metric Design.
...
## Output
Return a throughput constraint map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which queue signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready throughput constraint map tied to queue.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Queue Health Review, Agent Leverage Plan, Throughput Metric Design, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Throughput Constraint Map workflow for the following From Productivity to Throughput situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the throughput constraint map using the output schema, mark assumptions, and list the human review gates before anyone acts.
