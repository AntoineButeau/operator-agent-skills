...
## Use when
Use this workflow when you are working on From Productivity to Throughput and need to turn the diagnosis into an executable plan with owners and gates. Its distinct job inside the AI Throughput Pack is to produce the agent leverage plan; use the neighboring skills for Throughput Constraint Map, Queue Health Review, Throughput Metric Design.
...
- Domain signals to inspect: automation leverage, reusable context, review ratio, throughput gain, quality guardrail.
...
3. Inspect the domain mechanics for this pack: automation leverage, reusable context, review ratio, throughput gain, quality guardrail. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the agent leverage plan, and list what should be routed to Throughput Constraint Map, Queue Health Review, Throughput Metric Design.
...
## Output
Return a agent leverage plan with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which automation leverage signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready agent leverage plan tied to automation leverage.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Throughput Constraint Map, Queue Health Review, Throughput Metric Design, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Agent Leverage Plan workflow for the following From Productivity to Throughput situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the agent leverage plan using the output schema, mark assumptions, and list the human review gates before anyone acts.
