...
## Use when
Use this workflow when you are working on From Productivity to Throughput and need to design the operating artifact, interface, or workflow contract. Its distinct job inside the AI Throughput Pack is to produce the throughput metric design; use the neighboring skills for Throughput Constraint Map, Queue Health Review, Agent Leverage Plan.
...
- Domain signals to inspect: flow metric, quality countermetric, lead time, output definition, operating cadence.
...
3. Inspect the domain mechanics for this pack: flow metric, quality countermetric, lead time, output definition, operating cadence. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the throughput metric design, and list what should be routed to Throughput Constraint Map, Queue Health Review, Agent Leverage Plan.
...
## Output
Return a throughput metric design with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which flow metric signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready throughput metric design tied to flow metric.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Throughput Constraint Map, Queue Health Review, Agent Leverage Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Throughput Metric Design workflow for the following From Productivity to Throughput situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the throughput metric design using the output schema, mark assumptions, and list the human review gates before anyone acts.
