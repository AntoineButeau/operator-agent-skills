# First 90 Days Manager Diagnostic

## Use when
Use this workflow when you are working on expert manager and need to handle the diagnostic step without overlapping the other skills in the pack. Its distinct job inside the Manager Operating System Pack is to produce the first 90 days manager diagnostic; use the neighboring skills for Delegation Contract Builder, One-on-One System Designer, Decision Journal Coach.
...
- Domain signals to inspect: manager cascade, delegation contract, one-on-one cadence, decision journal, role expectations.
...
3. Inspect the domain mechanics for this pack: manager cascade, delegation contract, one-on-one cadence, decision journal, role expectations. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the first 90 days manager diagnostic, and list what should be routed to Delegation Contract Builder, One-on-One System Designer, Decision Journal Coach.
...
## Output
Return a first 90 days manager diagnostic with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which manager cascade signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready first 90 days manager diagnostic tied to manager cascade.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Delegation Contract Builder, One-on-One System Designer, Decision Journal Coach, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the First 90 Days Manager Diagnostic workflow for the following Expert Manager situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the first 90 days manager diagnostic using the output schema, mark assumptions, and list the human review gates before anyone acts.
