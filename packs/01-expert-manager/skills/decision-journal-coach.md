...
## Use when
Use this workflow when you are working on expert manager and need to handle the coach step without overlapping the other skills in the pack. Its distinct job inside the Manager Operating System Pack is to produce the decision journal coach; use the neighboring skills for First 90 Days Manager Diagnostic, Delegation Contract Builder, One-on-One System Designer.
...
4. Separate this skill's job from the rest of the pack: deliver the decision journal coach, and list what should be routed to First 90 Days Manager Diagnostic, Delegation Contract Builder, One-on-One System Designer.
...
## Output
Return a decision journal coach with this schema:
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready decision journal coach tied to manager cascade.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with First 90 Days Manager Diagnostic, Delegation Contract Builder, One-on-One System Designer, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Decision Journal Coach workflow for the following Expert Manager situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the decision journal coach using the output schema, mark assumptions, and list the human review gates before anyone acts.
