# One-on-One System Designer

## Use when
Use this workflow when you are working on expert manager and need to produce a reusable artifact or operating spec. Its distinct job inside the Manager Operating System Pack is to produce the implementation-ready spec; use the neighboring skills for First 90 Days Manager Diagnostic, Delegation Contract Builder, Decision Journal Coach.
...
4. Separate this skill's job from the rest of the pack: deliver the implementation-ready spec, and list what should be routed to First 90 Days Manager Diagnostic, Delegation Contract Builder, Decision Journal Coach.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready implementation-ready spec tied to manager cascade.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with First 90 Days Manager Diagnostic, Delegation Contract Builder, Decision Journal Coach, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the One-on-One System Designer workflow for the following Expert Manager situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the implementation-ready spec using the output schema, mark assumptions, and list the human review gates before anyone acts.
