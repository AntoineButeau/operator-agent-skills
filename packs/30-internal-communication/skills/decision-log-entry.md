...
## Use when
Use this workflow when you are working on internal communication that actually works: how companies move context, decisions, and signal without drowning in noise and need to handle the entry step without overlapping the other skills in the pack. Its distinct job inside the Company Signal Pack is to produce the decision log entry; use the neighboring skills for Company Signal Brief, Communication Channel Audit, Context Package Builder.
...
- Domain signals to inspect: decision log, context package, channel map, company signal, source of truth.
...
3. Inspect the domain mechanics for this pack: decision log, context package, channel map, company signal, source of truth. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the decision log entry, and list what should be routed to Company Signal Brief, Communication Channel Audit, Context Package Builder.
...
## Output
Return a decision log entry with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which decision log signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready decision log entry tied to decision log.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Company Signal Brief, Communication Channel Audit, Context Package Builder, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Decision Log Entry workflow for the following Internal Communication That Actually Works: How Companies Move Context, Decisions, and Signal Without Drowning in Noise situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the decision log entry using the output schema, mark assumptions, and list the human review gates before anyone acts.
