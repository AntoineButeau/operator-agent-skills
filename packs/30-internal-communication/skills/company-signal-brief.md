...
## Use when
Use this workflow when you are working on internal communication that actually works: how companies move context, decisions, and signal without drowning in noise and need to compress the situation into a decision-ready brief. Its distinct job inside the Company Signal Pack is to produce the operator memo; use the neighboring skills for Decision Log Entry, Communication Channel Audit, Context Package Builder.
...
4. Separate this skill's job from the rest of the pack: deliver the operator memo, and list what should be routed to Decision Log Entry, Communication Channel Audit, Context Package Builder.
...
## Output
Return a operator memo with this schema:
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready operator memo tied to decision log.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Decision Log Entry, Communication Channel Audit, Context Package Builder, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Company Signal Brief workflow for the following Internal Communication That Actually Works: How Companies Move Context, Decisions, and Signal Without Drowning in Noise situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the operator memo using the output schema, mark assumptions, and list the human review gates before anyone acts.
