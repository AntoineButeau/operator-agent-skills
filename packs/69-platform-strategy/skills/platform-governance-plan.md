# Platform Governance Plan

## Use when
Use this workflow when you are working on Platform Strategy and need to turn the diagnosis into an executable plan with owners and gates. Its distinct job inside the Platform Strategy Pack is to produce the governance plan; use the neighboring skills for Platform Boundary Map, Ecosystem Incentive Review, Platform Investment Brief.
...
- Domain signals to inspect: API policy, quality bar, extension review, deprecation, conflict.
...
3. Inspect the domain mechanics for this pack: API policy, quality bar, extension review, deprecation, conflict. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the governance plan, and list what should be routed to Platform Boundary Map, Ecosystem Incentive Review, Platform Investment Brief.
...
## Output
Return a governance plan with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which API policy signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready governance plan tied to API policy.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Platform Boundary Map, Ecosystem Incentive Review, Platform Investment Brief, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Platform Governance Plan workflow for the following Platform Strategy situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the governance plan using the output schema, mark assumptions, and list the human review gates before anyone acts.
