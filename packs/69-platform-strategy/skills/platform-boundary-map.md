...
## Use when
Use this workflow when you are working on Platform Strategy and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the Platform Strategy Pack is to produce the platform boundary map; use the neighboring skills for Ecosystem Incentive Review, Platform Investment Brief, Platform Governance Plan.
...
- Domain signals to inspect: core platform, extension point, tenant need, governance, dependency.
...
3. Inspect the domain mechanics for this pack: core platform, extension point, tenant need, governance, dependency. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the platform boundary map, and list what should be routed to Ecosystem Incentive Review, Platform Investment Brief, Platform Governance Plan.
...
## Output
Return a platform boundary map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which core platform signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready platform boundary map tied to core platform.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Ecosystem Incentive Review, Platform Investment Brief, Platform Governance Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Platform Boundary Map workflow for the following Platform Strategy situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the platform boundary map using the output schema, mark assumptions, and list the human review gates before anyone acts.
