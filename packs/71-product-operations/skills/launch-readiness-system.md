...
## Use when
Use this workflow when you are working on Product Operations That Actually Helps Product Teams Ship and need to define the repeatable operating system and cadence. Its distinct job inside the Product Operations Pack is to produce the launch readiness system; use the neighboring skills for Product Ops Intake Map, Product Feedback Routing, Product Team Operating Review.
...
- Domain signals to inspect: launch checklist, dependency, release owner, customer communication, rollback.
...
3. Inspect the domain mechanics for this pack: launch checklist, dependency, release owner, customer communication, rollback. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the launch readiness system, and list what should be routed to Product Ops Intake Map, Product Feedback Routing, Product Team Operating Review.
...
## Output
Return a launch readiness system with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which launch checklist signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready launch readiness system tied to launch checklist.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Product Ops Intake Map, Product Feedback Routing, Product Team Operating Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Launch Readiness System workflow for the following Product Operations That Actually Helps Product Teams Ship situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the launch readiness system using the output schema, mark assumptions, and list the human review gates before anyone acts.
