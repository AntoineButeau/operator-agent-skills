# Product Team Operating Review

## Use when
Use this workflow when you are working on Product Operations That Actually Helps Product Teams Ship and need to diagnose the current state and name the operating risk. Its distinct job inside the Product Operations Pack is to produce the operating review; use the neighboring skills for Product Ops Intake Map, Launch Readiness System, Product Feedback Routing.
...
- Domain signals to inspect: delivery metric, discovery signal, dependency, decision log, review cadence.
...
3. Inspect the domain mechanics for this pack: delivery metric, discovery signal, dependency, decision log, review cadence. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the operating review, and list what should be routed to Product Ops Intake Map, Launch Readiness System, Product Feedback Routing.
...
## Output
Return a operating review with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which delivery metric signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready operating review tied to delivery metric.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Product Ops Intake Map, Launch Readiness System, Product Feedback Routing, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Product Team Operating Review workflow for the following Product Operations That Actually Helps Product Teams Ship situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the operating review using the output schema, mark assumptions, and list the human review gates before anyone acts.
