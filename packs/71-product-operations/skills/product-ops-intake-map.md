# Product Ops Intake Map

## Use when
Use this workflow when you are working on Product Operations That Actually Helps Product Teams Ship and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the Product Operations Pack is to produce the intake map; use the neighboring skills for Launch Readiness System, Product Feedback Routing, Product Team Operating Review.
...
- Domain signals to inspect: request source, prioritization rule, triage owner, SLA, noise.
...
3. Inspect the domain mechanics for this pack: request source, prioritization rule, triage owner, SLA, noise. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the intake map, and list what should be routed to Launch Readiness System, Product Feedback Routing, Product Team Operating Review.
...
## Output
Return a intake map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which request source signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready intake map tied to request source.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Launch Readiness System, Product Feedback Routing, Product Team Operating Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Product Ops Intake Map workflow for the following Product Operations That Actually Helps Product Teams Ship situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the intake map using the output schema, mark assumptions, and list the human review gates before anyone acts.
