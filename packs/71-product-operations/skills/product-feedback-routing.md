...
## Use when
Use this workflow when you are working on Product Operations That Actually Helps Product Teams Ship and need to handle the system step without overlapping the other skills in the pack. Its distinct job inside the Product Operations Pack is to produce the feedback routing; use the neighboring skills for Product Ops Intake Map, Launch Readiness System, Product Team Operating Review.
...
- Domain signals to inspect: feedback source, taxonomy, severity, product owner, close loop.
...
3. Inspect the domain mechanics for this pack: feedback source, taxonomy, severity, product owner, close loop. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the feedback routing, and list what should be routed to Product Ops Intake Map, Launch Readiness System, Product Team Operating Review.
...
## Output
Return a feedback routing with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which feedback source signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready feedback routing tied to feedback source.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Product Ops Intake Map, Launch Readiness System, Product Team Operating Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Product Feedback Routing workflow for the following Product Operations That Actually Helps Product Teams Ship situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the feedback routing using the output schema, mark assumptions, and list the human review gates before anyone acts.
