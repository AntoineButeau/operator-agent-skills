# Automation ROI Sensitivity Review

## Use when
Use this workflow when you are working on The Economics of Intelligence and need to diagnose the current state and name the operating risk. Its distinct job inside the Intelligence Economics Pack is to produce the sensitivity review; use the neighboring skills for Intelligence Cost Curve Map, Build Buy Model Routing Brief, Intelligence Allocation Plan.
...
- Domain signals to inspect: adoption rate, error cost, latency gain, maintenance burden, margin impact.
...
3. Inspect the domain mechanics for this pack: adoption rate, error cost, latency gain, maintenance burden, margin impact. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the sensitivity review, and list what should be routed to Intelligence Cost Curve Map, Build Buy Model Routing Brief, Intelligence Allocation Plan.
...
## Output
Return a sensitivity review with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which adoption rate signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready sensitivity review tied to adoption rate.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Intelligence Cost Curve Map, Build Buy Model Routing Brief, Intelligence Allocation Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Automation ROI Sensitivity Review workflow for the following The Economics of Intelligence situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the sensitivity review using the output schema, mark assumptions, and list the human review gates before anyone acts.
