...
## Use when
Use this workflow when you are working on The Economics of Intelligence and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the Intelligence Economics Pack is to produce the cost curve map; use the neighboring skills for Build Buy Model Routing Brief, Automation ROI Sensitivity Review, Intelligence Allocation Plan.
...
- Domain signals to inspect: model cost, labor substitution, marginal analysis, quality threshold, scale effect.
...
3. Inspect the domain mechanics for this pack: model cost, labor substitution, marginal analysis, quality threshold, scale effect. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the cost curve map, and list what should be routed to Build Buy Model Routing Brief, Automation ROI Sensitivity Review, Intelligence Allocation Plan.
...
## Output
Return a cost curve map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which model cost signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready cost curve map tied to model cost.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Build Buy Model Routing Brief, Automation ROI Sensitivity Review, Intelligence Allocation Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Intelligence Cost Curve Map workflow for the following The Economics of Intelligence situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the cost curve map using the output schema, mark assumptions, and list the human review gates before anyone acts.
