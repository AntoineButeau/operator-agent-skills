# Cold Start Risk Scan

## Use when
Use this workflow when you are working on Network Effects That Actually Compound and need to surface risks, constraints, and missing evidence before action. Its distinct job inside the Network Effects Pack is to produce the cold-start risk scan; use the neighboring skills for Network Loop Map, Liquidity Metric Design, Defensibility Review.
...
- Domain signals to inspect: supply seed, demand density, activation gap, trust bootstrap, subsidy path.
...
3. Inspect the domain mechanics for this pack: supply seed, demand density, activation gap, trust bootstrap, subsidy path. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the cold-start risk scan, and list what should be routed to Network Loop Map, Liquidity Metric Design, Defensibility Review.
...
## Output
Return a cold-start risk scan with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which supply seed signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready cold-start risk scan tied to supply seed.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Network Loop Map, Liquidity Metric Design, Defensibility Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Cold Start Risk Scan workflow for the following Network Effects That Actually Compound situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the cold-start risk scan using the output schema, mark assumptions, and list the human review gates before anyone acts.
