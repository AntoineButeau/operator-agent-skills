...
## Use when
Use this workflow when you are working on Network Effects That Actually Compound and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the Network Effects Pack is to produce the network loop map; use the neighboring skills for Cold Start Risk Scan, Liquidity Metric Design, Defensibility Review.
...
- Domain signals to inspect: participant role, value loop, liquidity threshold, cross-side incentive, compounding signal.
...
3. Inspect the domain mechanics for this pack: participant role, value loop, liquidity threshold, cross-side incentive, compounding signal. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the network loop map, and list what should be routed to Cold Start Risk Scan, Liquidity Metric Design, Defensibility Review.
...
## Output
Return a network loop map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which participant role signal changed.
