...
## Use when
Use this workflow when you are working on Network Effects That Actually Compound and need to diagnose the current state and name the operating risk. Its distinct job inside the Network Effects Pack is to produce the defensibility review; use the neighboring skills for Network Loop Map, Cold Start Risk Scan, Liquidity Metric Design.
...
- Domain signals to inspect: switching cost, data advantage, reputation asset, scale effect, network decay.
...
3. Inspect the domain mechanics for this pack: switching cost, data advantage, reputation asset, scale effect, network decay. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the defensibility review, and list what should be routed to Network Loop Map, Cold Start Risk Scan, Liquidity Metric Design.
...
## Output
Return a defensibility review with this schema:
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready defensibility review tied to switching cost.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Network Loop Map, Cold Start Risk Scan, Liquidity Metric Design, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Defensibility Review workflow for the following Network Effects That Actually Compound situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the defensibility review using the output schema, mark assumptions, and list the human review gates before anyone acts.
