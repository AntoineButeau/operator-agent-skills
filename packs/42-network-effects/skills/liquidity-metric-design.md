# Liquidity Metric Design

## Use when
Use this workflow when you are working on Network Effects That Actually Compound and need to design the operating artifact, interface, or workflow contract. Its distinct job inside the Network Effects Pack is to produce the liquidity metric design; use the neighboring skills for Network Loop Map, Cold Start Risk Scan, Defensibility Review.
...
- Domain signals to inspect: match rate, response time, density metric, marketplace health, cohort behavior.
...
3. Inspect the domain mechanics for this pack: match rate, response time, density metric, marketplace health, cohort behavior. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the liquidity metric design, and list what should be routed to Network Loop Map, Cold Start Risk Scan, Defensibility Review.
...
## Output
Return a liquidity metric design with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which match rate signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready liquidity metric design tied to match rate.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Network Loop Map, Cold Start Risk Scan, Defensibility Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Liquidity Metric Design workflow for the following Network Effects That Actually Compound situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the liquidity metric design using the output schema, mark assumptions, and list the human review gates before anyone acts.
