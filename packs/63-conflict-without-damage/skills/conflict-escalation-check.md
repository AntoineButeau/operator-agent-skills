...
## Use when
Use this workflow when you are working on Conflict Without Damage and need to handle the system step without overlapping the other skills in the pack. Its distinct job inside the Conflict Without Damage Pack is to produce the escalation check; use the neighboring skills for Conflict Pattern Diagnosis, Clean Disagreement Brief, Repair Conversation Plan.
...
- Domain signals to inspect: decision impasse, authority path, mediation need, urgency, blast radius.
...
3. Inspect the domain mechanics for this pack: decision impasse, authority path, mediation need, urgency, blast radius. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the escalation check, and list what should be routed to Conflict Pattern Diagnosis, Clean Disagreement Brief, Repair Conversation Plan.
...
## Output
Return a escalation check with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which decision impasse signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready escalation check tied to decision impasse.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Conflict Pattern Diagnosis, Clean Disagreement Brief, Repair Conversation Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Conflict Escalation Check workflow for the following Conflict Without Damage situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the escalation check using the output schema, mark assumptions, and list the human review gates before anyone acts.
