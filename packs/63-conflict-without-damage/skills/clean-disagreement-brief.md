# Clean Disagreement Brief

## Use when
Use this workflow when you are working on Conflict Without Damage and need to produce a reusable decision brief or operating spec. Its distinct job inside the Conflict Without Damage Pack is to produce the disagreement brief; use the neighboring skills for Conflict Pattern Diagnosis, Repair Conversation Plan, Conflict Escalation Check.
...
- Domain signals to inspect: shared goal, point of difference, evidence, tradeoff, ask.
...
3. Inspect the domain mechanics for this pack: shared goal, point of difference, evidence, tradeoff, ask. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the disagreement brief, and list what should be routed to Conflict Pattern Diagnosis, Repair Conversation Plan, Conflict Escalation Check.
...
## Output
Return a disagreement brief with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which shared goal signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready disagreement brief tied to shared goal.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Conflict Pattern Diagnosis, Repair Conversation Plan, Conflict Escalation Check, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Clean Disagreement Brief workflow for the following Conflict Without Damage situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the disagreement brief using the output schema, mark assumptions, and list the human review gates before anyone acts.
