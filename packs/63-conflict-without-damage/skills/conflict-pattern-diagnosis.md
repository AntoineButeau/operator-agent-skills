# Conflict Pattern Diagnosis

## Use when
Use this workflow when you are working on Conflict Without Damage and need to handle the system step without overlapping the other skills in the pack. Its distinct job inside the Conflict Without Damage Pack is to produce the conflict pattern diagnosis; use the neighboring skills for Clean Disagreement Brief, Repair Conversation Plan, Conflict Escalation Check.
...
- Domain signals to inspect: disagreement type, trigger, escalation path, relationship risk, decision need.
...
3. Inspect the domain mechanics for this pack: disagreement type, trigger, escalation path, relationship risk, decision need. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the conflict pattern diagnosis, and list what should be routed to Clean Disagreement Brief, Repair Conversation Plan, Conflict Escalation Check.
...
## Output
Return a conflict pattern diagnosis with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which disagreement type signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready conflict pattern diagnosis tied to disagreement type.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Clean Disagreement Brief, Repair Conversation Plan, Conflict Escalation Check, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Conflict Pattern Diagnosis workflow for the following Conflict Without Damage situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the conflict pattern diagnosis using the output schema, mark assumptions, and list the human review gates before anyone acts.
