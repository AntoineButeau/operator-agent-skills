...
## Use when
Use this workflow when you are working on Taste Is the Bottleneck and need to handle the system step without overlapping the other skills in the pack. Its distinct job inside the Taste Bottleneck Pack is to produce the taste gap diagnosis; use the neighboring skills for Reference Set Builder, Quality Bar Review, Taste Calibration Loop.
...
- Domain signals to inspect: quality bar, reference set, judgment standard, user reaction, craft tradeoff.
...
3. Inspect the domain mechanics for this pack: quality bar, reference set, judgment standard, user reaction, craft tradeoff. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the taste gap diagnosis, and list what should be routed to Reference Set Builder, Quality Bar Review, Taste Calibration Loop.
...
## Output
Return a taste gap diagnosis with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which quality bar signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready taste gap diagnosis tied to quality bar.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Reference Set Builder, Quality Bar Review, Taste Calibration Loop, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Taste Gap Diagnosis workflow for the following Taste Is the Bottleneck situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the taste gap diagnosis using the output schema, mark assumptions, and list the human review gates before anyone acts.
