...
## Use when
Use this workflow when you are working on Taste Is the Bottleneck and need to handle the system step without overlapping the other skills in the pack. Its distinct job inside the Taste Bottleneck Pack is to produce the calibration loop; use the neighboring skills for Taste Gap Diagnosis, Reference Set Builder, Quality Bar Review.
...
- Domain signals to inspect: review ritual, comparison set, disagreement record, improvement target, taste transfer.
...
3. Inspect the domain mechanics for this pack: review ritual, comparison set, disagreement record, improvement target, taste transfer. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the calibration loop, and list what should be routed to Taste Gap Diagnosis, Reference Set Builder, Quality Bar Review.
...
## Output
Return a calibration loop with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which review ritual signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready calibration loop tied to review ritual.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Taste Gap Diagnosis, Reference Set Builder, Quality Bar Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Taste Calibration Loop workflow for the following Taste Is the Bottleneck situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the calibration loop using the output schema, mark assumptions, and list the human review gates before anyone acts.
