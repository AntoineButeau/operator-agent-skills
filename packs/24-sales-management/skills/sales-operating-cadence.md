...
## Use when
Use this workflow when you are working on sales management that actually works and need to handle the cadence step without overlapping the other skills in the pack. Its distinct job inside the Sales Manager OS Pack is to produce the sales operating cadence; use the neighboring skills for Pipeline Inspection Routine, Rep Coaching Plan, Forecast Call Prep.
...
4. Separate this skill's job from the rest of the pack: deliver the sales operating cadence, and list what should be routed to Pipeline Inspection Routine, Rep Coaching Plan, Forecast Call Prep.
...
## Output
Return a sales operating cadence with this schema:
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready sales operating cadence tied to forecast call.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Pipeline Inspection Routine, Rep Coaching Plan, Forecast Call Prep, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Sales Operating Cadence workflow for the following Sales Management That Actually Works situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the sales operating cadence using the output schema, mark assumptions, and list the human review gates before anyone acts.
