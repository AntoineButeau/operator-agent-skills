...
## Use when
Use this workflow when you are working on sales management that actually works and need to handle the prep step without overlapping the other skills in the pack. Its distinct job inside the Sales Manager OS Pack is to produce the revenue inspection brief; use the neighboring skills for Pipeline Inspection Routine, Rep Coaching Plan, Sales Operating Cadence.
...
- Domain signals to inspect: forecast call, pipeline inspection, rep coaching, sales cadence, deal risk.
...
3. Inspect the domain mechanics for this pack: forecast call, pipeline inspection, rep coaching, sales cadence, deal risk. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the revenue inspection brief, and list what should be routed to Pipeline Inspection Routine, Rep Coaching Plan, Sales Operating Cadence.
...
## Output
Return a revenue inspection brief with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which forecast call signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready revenue inspection brief tied to forecast call.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Pipeline Inspection Routine, Rep Coaching Plan, Sales Operating Cadence, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Forecast Call Prep workflow for the following Sales Management That Actually Works situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the revenue inspection brief using the output schema, mark assumptions, and list the human review gates before anyone acts.
