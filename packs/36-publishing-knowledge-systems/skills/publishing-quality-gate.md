...
## Use when
Use this workflow when you are working on publishing & knowledge systems and need to handle the gate step without overlapping the other skills in the pack. Its distinct job inside the Publishing Pipeline Pack is to produce the publishing quality gate; use the neighboring skills for Idea to Draft Pipeline, Editorial Calendar Builder, Knowledge Asset Inventory.
...
- Domain signals to inspect: editorial calendar, quality gate, knowledge asset, draft pipeline, publishing queue.
...
3. Inspect the domain mechanics for this pack: editorial calendar, quality gate, knowledge asset, draft pipeline, publishing queue. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the publishing quality gate, and list what should be routed to Idea to Draft Pipeline, Editorial Calendar Builder, Knowledge Asset Inventory.
...
## Output
Return a publishing quality gate with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which editorial calendar signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready publishing quality gate tied to editorial calendar.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Idea to Draft Pipeline, Editorial Calendar Builder, Knowledge Asset Inventory, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Publishing Quality Gate workflow for the following Publishing & Knowledge Systems situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the publishing quality gate using the output schema, mark assumptions, and list the human review gates before anyone acts.
