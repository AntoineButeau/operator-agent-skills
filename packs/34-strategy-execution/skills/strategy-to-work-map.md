# Strategy to Work Map

## Use when
Use this workflow when you are working on strategy execution and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the Strategy Translation Pack is to produce the owner-and-flow map; use the neighboring skills for Initiative Priority Check, Execution Dependency Map, Strategy Review Cadence.
...
4. Separate this skill's job from the rest of the pack: deliver the owner-and-flow map, and list what should be routed to Initiative Priority Check, Execution Dependency Map, Strategy Review Cadence.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready owner-and-flow map tied to strategy-to-work map.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Initiative Priority Check, Execution Dependency Map, Strategy Review Cadence, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Strategy to Work Map workflow for the following Strategy Execution situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the owner-and-flow map using the output schema, mark assumptions, and list the human review gates before anyone acts.
