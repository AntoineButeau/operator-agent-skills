# Truth Conflict Resolution Plan

## Use when
Use this workflow when you are working on Systems of Record: Who Owns the Truth? and need to turn the diagnosis into an executable plan with owners and gates. Its distinct job inside the Systems of Record Pack is to produce the conflict resolution plan; use the neighboring skills for Source of Truth Map, Record Ownership Review, System of Record Change Brief.
...
- Domain signals to inspect: conflicting record, precedence rule, reconciliation owner, exception process, communication.
...
3. Inspect the domain mechanics for this pack: conflicting record, precedence rule, reconciliation owner, exception process, communication. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the conflict resolution plan, and list what should be routed to Source of Truth Map, Record Ownership Review, System of Record Change Brief.
...
## Output
Return a conflict resolution plan with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which conflicting record signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready conflict resolution plan tied to conflicting record.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Source of Truth Map, Record Ownership Review, System of Record Change Brief, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Truth Conflict Resolution Plan workflow for the following Systems of Record: Who Owns the Truth? situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the conflict resolution plan using the output schema, mark assumptions, and list the human review gates before anyone acts.
