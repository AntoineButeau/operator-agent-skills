# Record Ownership Review

## Use when
Use this workflow when you are working on Systems of Record: Who Owns the Truth? and need to diagnose the current state and name the operating risk. Its distinct job inside the Systems of Record Pack is to produce the ownership review; use the neighboring skills for Source of Truth Map, Truth Conflict Resolution Plan, System of Record Change Brief.
...
- Domain signals to inspect: data steward, decision right, record lifecycle, audit need, handoff.
...
3. Inspect the domain mechanics for this pack: data steward, decision right, record lifecycle, audit need, handoff. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the ownership review, and list what should be routed to Source of Truth Map, Truth Conflict Resolution Plan, System of Record Change Brief.
...
## Output
Return a ownership review with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which data steward signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready ownership review tied to data steward.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Source of Truth Map, Truth Conflict Resolution Plan, System of Record Change Brief, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Record Ownership Review workflow for the following Systems of Record: Who Owns the Truth? situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the ownership review using the output schema, mark assumptions, and list the human review gates before anyone acts.
