...
## Use when
Use this workflow when you are working on Systems of Record: Who Owns the Truth? and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the Systems of Record Pack is to produce the source-of-truth map; use the neighboring skills for Record Ownership Review, Truth Conflict Resolution Plan, System of Record Change Brief.
...
- Domain signals to inspect: record owner, system boundary, update path, duplicate source, conflict rule.
...
3. Inspect the domain mechanics for this pack: record owner, system boundary, update path, duplicate source, conflict rule. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the source-of-truth map, and list what should be routed to Record Ownership Review, Truth Conflict Resolution Plan, System of Record Change Brief.
...
## Output
Return a source-of-truth map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which record owner signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready source-of-truth map tied to record owner.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Record Ownership Review, Truth Conflict Resolution Plan, System of Record Change Brief, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Source of Truth Map workflow for the following Systems of Record: Who Owns the Truth? situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the source-of-truth map using the output schema, mark assumptions, and list the human review gates before anyone acts.
