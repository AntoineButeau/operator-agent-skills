...
## Use when
Use this workflow when you are working on automation / ai automation architecture and need to produce a reusable artifact or operating spec. Its distinct job inside the Automation Design Pack is to produce the human-in-the-loop design; use the neighboring skills for Automation Opportunity Scan, Automation ROI Brief, Agent Workflow Spec.
...
- Domain signals to inspect: agent workflow, human-in-the-loop, exception queue, automation ROI, approval gate.
...
3. Inspect the domain mechanics for this pack: agent workflow, human-in-the-loop, exception queue, automation ROI, approval gate. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the human-in-the-loop design, and list what should be routed to Automation Opportunity Scan, Automation ROI Brief, Agent Workflow Spec.
...
## Output
Return a human-in-the-loop design with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which agent workflow signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready human-in-the-loop design tied to agent workflow.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Automation Opportunity Scan, Automation ROI Brief, Agent Workflow Spec, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Human-in-the-Loop Design workflow for the following Automation / AI Automation Architecture situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the human-in-the-loop design using the output schema, mark assumptions, and list the human review gates before anyone acts.
