...
## Use when
Use this workflow when you are working on Trust That Actually Compounds and need to handle the system step without overlapping the other skills in the pack. Its distinct job inside the Trust Compounding Pack is to produce the trust loop; use the neighboring skills for Trust Signal Map, Commitment Reliability Review, Trust Repair Plan.
...
- Domain signals to inspect: repeatable behavior, signal cadence, mutual risk, escalation norm, credibility asset.
...
3. Inspect the domain mechanics for this pack: repeatable behavior, signal cadence, mutual risk, escalation norm, credibility asset. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the trust loop, and list what should be routed to Trust Signal Map, Commitment Reliability Review, Trust Repair Plan.
...
## Output
Return a trust loop with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which repeatable behavior signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready trust loop tied to repeatable behavior.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Trust Signal Map, Commitment Reliability Review, Trust Repair Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Compounding Trust Loop workflow for the following Trust That Actually Compounds situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the trust loop using the output schema, mark assumptions, and list the human review gates before anyone acts.
