...
## Use when
Use this workflow when you are working on Trust That Actually Compounds and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the Trust Compounding Pack is to produce the trust signal map; use the neighboring skills for Commitment Reliability Review, Trust Repair Plan, Compounding Trust Loop.
...
- Domain signals to inspect: promise, followthrough, transparency, competence signal, repair moment.
...
3. Inspect the domain mechanics for this pack: promise, followthrough, transparency, competence signal, repair moment. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the trust signal map, and list what should be routed to Commitment Reliability Review, Trust Repair Plan, Compounding Trust Loop.
...
## Output
Return a trust signal map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which promise signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready trust signal map tied to promise.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Commitment Reliability Review, Trust Repair Plan, Compounding Trust Loop, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Trust Signal Map workflow for the following Trust That Actually Compounds situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the trust signal map using the output schema, mark assumptions, and list the human review gates before anyone acts.
