# Compounding Channel Evaluator

## Use when
Use this workflow when you are working on growth systems that actually compound and need to handle the evaluator step without overlapping the other skills in the pack. Its distinct job inside the Growth Loop Pack is to produce the compounding channel evaluator; use the neighboring skills for Growth Loop Mapper, Experiment Portfolio Review, Retention Lever Scan.
...
- Domain signals to inspect: growth loop, retention lever, experiment portfolio, compounding channel, cohort signal.
...
3. Inspect the domain mechanics for this pack: growth loop, retention lever, experiment portfolio, compounding channel, cohort signal. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the compounding channel evaluator, and list what should be routed to Growth Loop Mapper, Experiment Portfolio Review, Retention Lever Scan.
...
## Output
Return a compounding channel evaluator with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which growth loop signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready compounding channel evaluator tied to growth loop.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Growth Loop Mapper, Experiment Portfolio Review, Retention Lever Scan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Compounding Channel Evaluator workflow for the following Growth Systems That Actually Compound situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the compounding channel evaluator using the output schema, mark assumptions, and list the human review gates before anyone acts.
