...
## Use when
Use this workflow when you are working on ai-native gtm and need to handle the system step without overlapping the other skills in the pack. Its distinct job inside the AI-Native GTM Pack is to produce the ai-native campaign system; use the neighboring skills for AI-Native Buyer Journey Map, Agentic Sales Workflow Spec, GTM Data Readiness Scan.
...
4. Separate this skill's job from the rest of the pack: deliver the ai-native campaign system, and list what should be routed to AI-Native Buyer Journey Map, Agentic Sales Workflow Spec, GTM Data Readiness Scan.
...
## Output
Return a ai-native campaign system with this schema:
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready ai-native campaign system tied to agentic sales workflow.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with AI-Native Buyer Journey Map, Agentic Sales Workflow Spec, GTM Data Readiness Scan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the AI-Native Campaign System workflow for the following AI-Native GTM situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the ai-native campaign system using the output schema, mark assumptions, and list the human review gates before anyone acts.
