# Hidden Data Model Map

## Use when
Use this workflow when you are working on The Company’s Hidden Data Model and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the Hidden Data Model Pack is to produce the hidden data model map; use the neighboring skills for Entity Ownership Audit, Shadow Data Reconciliation Plan, Decision Data Fit Review.
...
- Domain signals to inspect: entity definition, source-of-truth gap, shadow spreadsheet, workflow dependency, semantic drift.
...
3. Inspect the domain mechanics for this pack: entity definition, source-of-truth gap, shadow spreadsheet, workflow dependency, semantic drift. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the hidden data model map, and list what should be routed to Entity Ownership Audit, Shadow Data Reconciliation Plan, Decision Data Fit Review.
...
## Output
Return a hidden data model map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which entity definition signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready hidden data model map tied to entity definition.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Entity Ownership Audit, Shadow Data Reconciliation Plan, Decision Data Fit Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Hidden Data Model Map workflow for the following The Company’s Hidden Data Model situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the hidden data model map using the output schema, mark assumptions, and list the human review gates before anyone acts.
