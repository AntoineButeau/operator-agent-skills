# Entity Ownership Audit

## Use when
Use this workflow when you are working on The Company’s Hidden Data Model and need to find gaps between stated process and observed behavior. Its distinct job inside the Hidden Data Model Pack is to produce the entity ownership audit; use the neighboring skills for Hidden Data Model Map, Shadow Data Reconciliation Plan, Decision Data Fit Review.
...
- Domain signals to inspect: customer object, account hierarchy, lifecycle state, data steward, update right.
...
3. Inspect the domain mechanics for this pack: customer object, account hierarchy, lifecycle state, data steward, update right. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the entity ownership audit, and list what should be routed to Hidden Data Model Map, Shadow Data Reconciliation Plan, Decision Data Fit Review.
...
## Output
Return a entity ownership audit with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which customer object signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready entity ownership audit tied to customer object.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Hidden Data Model Map, Shadow Data Reconciliation Plan, Decision Data Fit Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Entity Ownership Audit workflow for the following The Company’s Hidden Data Model situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the entity ownership audit using the output schema, mark assumptions, and list the human review gates before anyone acts.
