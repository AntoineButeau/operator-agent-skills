...
## Use when
Use this workflow when you are working on The Company’s Hidden Data Model and need to turn the diagnosis into an executable plan with owners and gates. Its distinct job inside the Hidden Data Model Pack is to produce the reconciliation plan; use the neighboring skills for Hidden Data Model Map, Entity Ownership Audit, Decision Data Fit Review.
...
- Domain signals to inspect: spreadsheet fork, manual override, duplicate record, transformation rule, stale field.
...
3. Inspect the domain mechanics for this pack: spreadsheet fork, manual override, duplicate record, transformation rule, stale field. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the reconciliation plan, and list what should be routed to Hidden Data Model Map, Entity Ownership Audit, Decision Data Fit Review.
...
## Output
Return a reconciliation plan with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which spreadsheet fork signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready reconciliation plan tied to spreadsheet fork.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Hidden Data Model Map, Entity Ownership Audit, Decision Data Fit Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Shadow Data Reconciliation Plan workflow for the following The Company’s Hidden Data Model situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the reconciliation plan using the output schema, mark assumptions, and list the human review gates before anyone acts.
