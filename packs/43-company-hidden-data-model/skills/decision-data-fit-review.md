# Decision Data Fit Review

## Use when
Use this workflow when you are working on The Company’s Hidden Data Model and need to diagnose the current state and name the operating risk. Its distinct job inside the Hidden Data Model Pack is to produce the decision-data fit review; use the neighboring skills for Hidden Data Model Map, Entity Ownership Audit, Shadow Data Reconciliation Plan.
...
- Domain signals to inspect: decision question, data lineage, field reliability, interpretation risk, operating metric.
...
3. Inspect the domain mechanics for this pack: decision question, data lineage, field reliability, interpretation risk, operating metric. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the decision-data fit review, and list what should be routed to Hidden Data Model Map, Entity Ownership Audit, Shadow Data Reconciliation Plan.
...
## Output
Return a decision-data fit review with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which decision question signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready decision-data fit review tied to decision question.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Hidden Data Model Map, Entity Ownership Audit, Shadow Data Reconciliation Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Decision Data Fit Review workflow for the following The Company’s Hidden Data Model situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the decision-data fit review using the output schema, mark assumptions, and list the human review gates before anyone acts.
