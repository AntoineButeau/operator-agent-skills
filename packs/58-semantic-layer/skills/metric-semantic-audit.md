...
## Use when
Use this workflow when you are working on The Semantic Layer: Where Business Meaning Becomes Infrastructure and need to find gaps between stated process and observed behavior. Its distinct job inside the Semantic Layer Pack is to produce the semantic audit; use the neighboring skills for Business Term Definition Map, Semantic Layer Change Plan, Meaning to Infrastructure Brief.
...
- Domain signals to inspect: metric drift, dashboard mismatch, denominator, grain, interpretation risk.
...
3. Inspect the domain mechanics for this pack: metric drift, dashboard mismatch, denominator, grain, interpretation risk. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the semantic audit, and list what should be routed to Business Term Definition Map, Semantic Layer Change Plan, Meaning to Infrastructure Brief.
...
## Output
Return a semantic audit with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which metric drift signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready semantic audit tied to metric drift.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Business Term Definition Map, Semantic Layer Change Plan, Meaning to Infrastructure Brief, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Metric Semantic Audit workflow for the following The Semantic Layer: Where Business Meaning Becomes Infrastructure situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the semantic audit using the output schema, mark assumptions, and list the human review gates before anyone acts.
