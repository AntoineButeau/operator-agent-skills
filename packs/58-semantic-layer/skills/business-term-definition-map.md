# Business Term Definition Map

## Use when
Use this workflow when you are working on The Semantic Layer: Where Business Meaning Becomes Infrastructure and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the Semantic Layer Pack is to produce the term definition map; use the neighboring skills for Metric Semantic Audit, Semantic Layer Change Plan, Meaning to Infrastructure Brief.
...
- Domain signals to inspect: metric definition, business object, semantic owner, calculation rule, exception.
...
3. Inspect the domain mechanics for this pack: metric definition, business object, semantic owner, calculation rule, exception. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the term definition map, and list what should be routed to Metric Semantic Audit, Semantic Layer Change Plan, Meaning to Infrastructure Brief.
...
## Output
Return a term definition map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which metric definition signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready term definition map tied to metric definition.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Metric Semantic Audit, Semantic Layer Change Plan, Meaning to Infrastructure Brief, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Business Term Definition Map workflow for the following The Semantic Layer: Where Business Meaning Becomes Infrastructure situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the term definition map using the output schema, mark assumptions, and list the human review gates before anyone acts.
