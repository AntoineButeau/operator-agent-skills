# Meaning to Infrastructure Brief

## Use when
Use this workflow when you are working on The Semantic Layer: Where Business Meaning Becomes Infrastructure and need to produce a reusable decision brief or operating spec. Its distinct job inside the Semantic Layer Pack is to produce the infrastructure brief; use the neighboring skills for Business Term Definition Map, Metric Semantic Audit, Semantic Layer Change Plan.
...
- Domain signals to inspect: business meaning, model field, transform, API contract, data product.
...
3. Inspect the domain mechanics for this pack: business meaning, model field, transform, API contract, data product. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the infrastructure brief, and list what should be routed to Business Term Definition Map, Metric Semantic Audit, Semantic Layer Change Plan.
...
## Output
Return a infrastructure brief with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which business meaning signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready infrastructure brief tied to business meaning.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Business Term Definition Map, Metric Semantic Audit, Semantic Layer Change Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Meaning to Infrastructure Brief workflow for the following The Semantic Layer: Where Business Meaning Becomes Infrastructure situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the infrastructure brief using the output schema, mark assumptions, and list the human review gates before anyone acts.
