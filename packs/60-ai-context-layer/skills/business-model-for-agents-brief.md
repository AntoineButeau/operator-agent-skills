# Business Model for Agents Brief

## Use when
Use this workflow when you are working on The AI Context Layer: Giving Agents a Business Model They Can Safely Use and need to produce a reusable decision brief or operating spec. Its distinct job inside the AI Context Layer Pack is to produce the agent business model brief; use the neighboring skills for Agent Context Inventory, Context Contract Design, Context Risk Review.
...
- Domain signals to inspect: entity relationship, operating rule, authority boundary, memory policy, review cadence.
...
3. Inspect the domain mechanics for this pack: entity relationship, operating rule, authority boundary, memory policy, review cadence. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the agent business model brief, and list what should be routed to Agent Context Inventory, Context Contract Design, Context Risk Review.
...
## Output
Return a agent business model brief with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which entity relationship signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready agent business model brief tied to entity relationship.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Agent Context Inventory, Context Contract Design, Context Risk Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Business Model for Agents Brief workflow for the following The AI Context Layer: Giving Agents a Business Model They Can Safely Use situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the agent business model brief using the output schema, mark assumptions, and list the human review gates before anyone acts.
