...
## Use when
Use this workflow when you are working on The AI Context Layer: Giving Agents a Business Model They Can Safely Use and need to handle the system step without overlapping the other skills in the pack. Its distinct job inside the AI Context Layer Pack is to produce the context contract; use the neighboring skills for Agent Context Inventory, Context Risk Review, Business Model for Agents Brief.
...
- Domain signals to inspect: retrieval scope, freshness, sensitivity, provenance, fallback behavior.
...
3. Inspect the domain mechanics for this pack: retrieval scope, freshness, sensitivity, provenance, fallback behavior. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the context contract, and list what should be routed to Agent Context Inventory, Context Risk Review, Business Model for Agents Brief.
...
## Output
Return a context contract with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which retrieval scope signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready context contract tied to retrieval scope.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Agent Context Inventory, Context Risk Review, Business Model for Agents Brief, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Context Contract Design workflow for the following The AI Context Layer: Giving Agents a Business Model They Can Safely Use situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the context contract using the output schema, mark assumptions, and list the human review gates before anyone acts.
