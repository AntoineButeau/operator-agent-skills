...
## Use when
Use this workflow when you are working on The AI Context Layer: Giving Agents a Business Model They Can Safely Use and need to diagnose the current state and name the operating risk. Its distinct job inside the AI Context Layer Pack is to produce the context risk review; use the neighboring skills for Agent Context Inventory, Context Contract Design, Business Model for Agents Brief.
...
- Domain signals to inspect: stale context, overexposure, missing provenance, conflicting sources, unsafe inference.
...
3. Inspect the domain mechanics for this pack: stale context, overexposure, missing provenance, conflicting sources, unsafe inference. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the context risk review, and list what should be routed to Agent Context Inventory, Context Contract Design, Business Model for Agents Brief.
...
## Output
Return a context risk review with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which stale context signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready context risk review tied to stale context.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Agent Context Inventory, Context Contract Design, Business Model for Agents Brief, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Context Risk Review workflow for the following The AI Context Layer: Giving Agents a Business Model They Can Safely Use situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the context risk review using the output schema, mark assumptions, and list the human review gates before anyone acts.
