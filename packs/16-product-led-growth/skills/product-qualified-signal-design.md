# Product-Qualified Signal Design

## Use when
Use this workflow when you are working on product-led growth and need to produce a reusable artifact or operating spec. Its distinct job inside the PLG Operating Pack is to produce the product-qualified signal design; use the neighboring skills for Activation Path Audit, Self-Serve Conversion Map, PLG Experiment Brief.
...
- Domain signals to inspect: activation path, PQL, self-serve funnel, conversion map, PLG experiment.
...
3. Inspect the domain mechanics for this pack: activation path, PQL, self-serve funnel, conversion map, PLG experiment. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the product-qualified signal design, and list what should be routed to Activation Path Audit, Self-Serve Conversion Map, PLG Experiment Brief.
...
## Output
Return a product-qualified signal design with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which activation path signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready product-qualified signal design tied to activation path.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Activation Path Audit, Self-Serve Conversion Map, PLG Experiment Brief, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Product-Qualified Signal Design workflow for the following Product-Led Growth situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the product-qualified signal design using the output schema, mark assumptions, and list the human review gates before anyone acts.
