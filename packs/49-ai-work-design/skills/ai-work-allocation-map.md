# AI Work Allocation Map

## Use when
Use this workflow when you are working on Work Design for the AI Era and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the AI Work Design Pack is to produce the work allocation map; use the neighboring skills for Human-Agent Handoff Design, Role Redesign Brief, Work Design Risk Review.
...
- Domain signals to inspect: human judgment, agent task, review point, context need, failure cost.
...
3. Inspect the domain mechanics for this pack: human judgment, agent task, review point, context need, failure cost. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the work allocation map, and list what should be routed to Human-Agent Handoff Design, Role Redesign Brief, Work Design Risk Review.
...
## Output
Return a work allocation map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which human judgment signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready work allocation map tied to human judgment.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Human-Agent Handoff Design, Role Redesign Brief, Work Design Risk Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the AI Work Allocation Map workflow for the following Work Design for the AI Era situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the work allocation map using the output schema, mark assumptions, and list the human review gates before anyone acts.
