...
## Use when
Use this workflow when you are working on personal productivity + knowledge management and need to handle the triage step without overlapping the other skills in the pack. Its distinct job inside the Personal Knowledge OS Pack is to produce the system decay triage; use the neighboring skills for Capture Purpose Filter, Weekly Review Protocol, Notes to Output Pipeline.
...
4. Separate this skill's job from the rest of the pack: deliver the system decay triage, and list what should be routed to Capture Purpose Filter, Weekly Review Protocol, Notes to Output Pipeline.
...
## Output
Return a system decay triage with this schema:
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready system decay triage tied to capture inbox.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Capture Purpose Filter, Weekly Review Protocol, Notes to Output Pipeline, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the System Decay Triage workflow for the following Personal Productivity + Knowledge Management situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the system decay triage using the output schema, mark assumptions, and list the human review gates before anyone acts.
