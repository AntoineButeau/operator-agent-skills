# Notes to Output Pipeline

## Use when
Use this workflow when you are working on personal productivity + knowledge management and need to handle the pipeline step without overlapping the other skills in the pack. Its distinct job inside the Personal Knowledge OS Pack is to produce the revenue inspection brief; use the neighboring skills for Capture Purpose Filter, Weekly Review Protocol, System Decay Triage.
...
- Domain signals to inspect: capture inbox, weekly review, source of truth, output pipeline, system decay.
...
3. Inspect the domain mechanics for this pack: capture inbox, weekly review, source of truth, output pipeline, system decay. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the revenue inspection brief, and list what should be routed to Capture Purpose Filter, Weekly Review Protocol, System Decay Triage.
...
- `diagnosis`: what is happening, why it matters, and which capture inbox signal changed.
