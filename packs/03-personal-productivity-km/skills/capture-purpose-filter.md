...
## Use when
Use this workflow when you are working on personal productivity + knowledge management and need to handle the filter step without overlapping the other skills in the pack. Its distinct job inside the Personal Knowledge OS Pack is to produce the capture purpose filter; use the neighboring skills for Weekly Review Protocol, Notes to Output Pipeline, System Decay Triage.
...
4. Separate this skill's job from the rest of the pack: deliver the capture purpose filter, and list what should be routed to Weekly Review Protocol, Notes to Output Pipeline, System Decay Triage.
...
## Output
Return a capture purpose filter with this schema:
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready capture purpose filter tied to capture inbox.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Weekly Review Protocol, Notes to Output Pipeline, System Decay Triage, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Capture Purpose Filter workflow for the following Personal Productivity + Knowledge Management situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the capture purpose filter using the output schema, mark assumptions, and list the human review gates before anyone acts.
