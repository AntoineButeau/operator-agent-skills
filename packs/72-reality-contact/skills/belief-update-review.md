...
## Use when
Use this workflow when you are working on Reality Contact: Escaping the Introspection Trap and need to diagnose the current state and name the operating risk. Its distinct job inside the Reality Contact Pack is to produce the belief update review; use the neighboring skills for Introspection Trap Diagnosis, External Signal Plan, Reality Contact Cadence.
...
- Domain signals to inspect: prior belief, new evidence, confidence shift, implication, next test.
...
3. Inspect the domain mechanics for this pack: prior belief, new evidence, confidence shift, implication, next test. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the belief update review, and list what should be routed to Introspection Trap Diagnosis, External Signal Plan, Reality Contact Cadence.
...
## Output
Return a belief update review with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which prior belief signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready belief update review tied to prior belief.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Introspection Trap Diagnosis, External Signal Plan, Reality Contact Cadence, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Belief Update Review workflow for the following Reality Contact: Escaping the Introspection Trap situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the belief update review using the output schema, mark assumptions, and list the human review gates before anyone acts.
