# Talent Bar Definition

## Use when
Use this workflow when you are working on People Systems That Actually Raise the Bar and need to handle the system step without overlapping the other skills in the pack. Its distinct job inside the People Systems Pack is to produce the talent bar definition; use the neighboring skills for Hiring Signal Scorecard, Performance System Review, Promotion Calibration Brief.
...
- Domain signals to inspect: role standard, evidence marker, calibration, performance expectation, values signal.
...
3. Inspect the domain mechanics for this pack: role standard, evidence marker, calibration, performance expectation, values signal. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the talent bar definition, and list what should be routed to Hiring Signal Scorecard, Performance System Review, Promotion Calibration Brief.
...
## Output
Return a talent bar definition with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which role standard signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready talent bar definition tied to role standard.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Hiring Signal Scorecard, Performance System Review, Promotion Calibration Brief, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Talent Bar Definition workflow for the following People Systems That Actually Raise the Bar situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the talent bar definition using the output schema, mark assumptions, and list the human review gates before anyone acts.
