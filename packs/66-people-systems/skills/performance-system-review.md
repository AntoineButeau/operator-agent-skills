...
## Use when
Use this workflow when you are working on People Systems That Actually Raise the Bar and need to diagnose the current state and name the operating risk. Its distinct job inside the People Systems Pack is to produce the performance system review; use the neighboring skills for Talent Bar Definition, Hiring Signal Scorecard, Promotion Calibration Brief.
...
- Domain signals to inspect: feedback cadence, rating standard, manager judgment, growth path, fairness risk.
...
3. Inspect the domain mechanics for this pack: feedback cadence, rating standard, manager judgment, growth path, fairness risk. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the performance system review, and list what should be routed to Talent Bar Definition, Hiring Signal Scorecard, Promotion Calibration Brief.
...
## Output
Return a performance system review with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which feedback cadence signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready performance system review tied to feedback cadence.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Talent Bar Definition, Hiring Signal Scorecard, Promotion Calibration Brief, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Performance System Review workflow for the following People Systems That Actually Raise the Bar situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the performance system review using the output schema, mark assumptions, and list the human review gates before anyone acts.
