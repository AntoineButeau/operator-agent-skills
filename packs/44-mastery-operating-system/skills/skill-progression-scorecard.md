# Skill Progression Scorecard

## Use when
Use this workflow when you are working on Mastery Is an Operating System and need to define evaluation criteria and evidence standards. Its distinct job inside the Mastery Operating System Pack is to produce the progression scorecard; use the neighboring skills for Mastery Loop Map, Deliberate Practice Plan, Feedback Quality Review.
...
- Domain signals to inspect: skill level, evidence marker, next edge, plateau risk, transfer test.
...
3. Inspect the domain mechanics for this pack: skill level, evidence marker, next edge, plateau risk, transfer test. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the progression scorecard, and list what should be routed to Mastery Loop Map, Deliberate Practice Plan, Feedback Quality Review.
...
## Output
Return a progression scorecard with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which skill level signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready progression scorecard tied to skill level.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Mastery Loop Map, Deliberate Practice Plan, Feedback Quality Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Skill Progression Scorecard workflow for the following Mastery Is an Operating System situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the progression scorecard using the output schema, mark assumptions, and list the human review gates before anyone acts.
