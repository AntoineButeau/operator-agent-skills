...
## Use when
Use this workflow when you are working on Product Strategy That Actually Makes Choices and need to diagnose the current state and name the operating risk. Its distinct job inside the Product Strategy Choices Pack is to produce the strategy fit review; use the neighboring skills for Product Choice Map, Roadmap Tradeoff Brief, Choice Communication Plan.
...
- Domain signals to inspect: product bet, company thesis, market pressure, capability, differentiation.
...
3. Inspect the domain mechanics for this pack: product bet, company thesis, market pressure, capability, differentiation. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the strategy fit review, and list what should be routed to Product Choice Map, Roadmap Tradeoff Brief, Choice Communication Plan.
...
## Output
Return a strategy fit review with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which product bet signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready strategy fit review tied to product bet.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Product Choice Map, Roadmap Tradeoff Brief, Choice Communication Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Strategy Fit Review workflow for the following Product Strategy That Actually Makes Choices situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the strategy fit review using the output schema, mark assumptions, and list the human review gates before anyone acts.
