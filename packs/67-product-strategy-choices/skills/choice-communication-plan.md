# Choice Communication Plan

## Use when
Use this workflow when you are working on Product Strategy That Actually Makes Choices and need to turn the diagnosis into an executable plan with owners and gates. Its distinct job inside the Product Strategy Choices Pack is to produce the choice communication plan; use the neighboring skills for Product Choice Map, Roadmap Tradeoff Brief, Strategy Fit Review.
...
- Domain signals to inspect: decision rationale, audience, narrative, objection, commitment.
...
3. Inspect the domain mechanics for this pack: decision rationale, audience, narrative, objection, commitment. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the choice communication plan, and list what should be routed to Product Choice Map, Roadmap Tradeoff Brief, Strategy Fit Review.
...
## Output
Return a choice communication plan with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which decision rationale signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready choice communication plan tied to decision rationale.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Product Choice Map, Roadmap Tradeoff Brief, Strategy Fit Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Choice Communication Plan workflow for the following Product Strategy That Actually Makes Choices situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the choice communication plan using the output schema, mark assumptions, and list the human review gates before anyone acts.
