# Roadmap Tradeoff Brief

## Use when
Use this workflow when you are working on Product Strategy That Actually Makes Choices and need to produce a reusable decision brief or operating spec. Its distinct job inside the Product Strategy Choices Pack is to produce the tradeoff brief; use the neighboring skills for Product Choice Map, Strategy Fit Review, Choice Communication Plan.
...
- Domain signals to inspect: roadmap option, customer impact, technical cost, strategy fit, timing.
...
3. Inspect the domain mechanics for this pack: roadmap option, customer impact, technical cost, strategy fit, timing. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the tradeoff brief, and list what should be routed to Product Choice Map, Strategy Fit Review, Choice Communication Plan.
...
## Output
Return a tradeoff brief with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which roadmap option signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready tradeoff brief tied to roadmap option.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Product Choice Map, Strategy Fit Review, Choice Communication Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Roadmap Tradeoff Brief workflow for the following Product Strategy That Actually Makes Choices situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the tradeoff brief using the output schema, mark assumptions, and list the human review gates before anyone acts.
