...
## Use when
Use this workflow when you are working on Product Strategy That Actually Makes Choices and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the Product Strategy Choices Pack is to produce the product choice map; use the neighboring skills for Roadmap Tradeoff Brief, Strategy Fit Review, Choice Communication Plan.
...
- Domain signals to inspect: strategic choice, customer segment, tradeoff, opportunity cost, success metric.
...
3. Inspect the domain mechanics for this pack: strategic choice, customer segment, tradeoff, opportunity cost, success metric. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the product choice map, and list what should be routed to Roadmap Tradeoff Brief, Strategy Fit Review, Choice Communication Plan.
...
## Output
Return a product choice map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which strategic choice signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready product choice map tied to strategic choice.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Roadmap Tradeoff Brief, Strategy Fit Review, Choice Communication Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Product Choice Map workflow for the following Product Strategy That Actually Makes Choices situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the product choice map using the output schema, mark assumptions, and list the human review gates before anyone acts.
