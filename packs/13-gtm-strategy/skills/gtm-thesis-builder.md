...
## Use when
Use this workflow when you are working on go-to-market strategy and need to produce a reusable artifact or operating spec. Its distinct job inside the GTM Strategy Pack is to produce the implementation-ready spec; use the neighboring skills for ICP Focus Check, Channel Fit Evaluator, GTM Operating Plan.
...
4. Separate this skill's job from the rest of the pack: deliver the implementation-ready spec, and list what should be routed to ICP Focus Check, Channel Fit Evaluator, GTM Operating Plan.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready implementation-ready spec tied to ICP.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with ICP Focus Check, Channel Fit Evaluator, GTM Operating Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the GTM Thesis Builder workflow for the following Go-To-Market Strategy situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the implementation-ready spec using the output schema, mark assumptions, and list the human review gates before anyone acts.
