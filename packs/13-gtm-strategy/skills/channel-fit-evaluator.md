# Channel Fit Evaluator

## Use when
Use this workflow when you are working on go-to-market strategy and need to handle the evaluator step without overlapping the other skills in the pack. Its distinct job inside the GTM Strategy Pack is to produce the channel fit evaluator; use the neighboring skills for GTM Thesis Builder, ICP Focus Check, GTM Operating Plan.
...
4. Separate this skill's job from the rest of the pack: deliver the channel fit evaluator, and list what should be routed to GTM Thesis Builder, ICP Focus Check, GTM Operating Plan.
...
## Output
Return a channel fit evaluator with this schema:
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready channel fit evaluator tied to ICP.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with GTM Thesis Builder, ICP Focus Check, GTM Operating Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Channel Fit Evaluator workflow for the following Go-To-Market Strategy situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the channel fit evaluator using the output schema, mark assumptions, and list the human review gates before anyone acts.
