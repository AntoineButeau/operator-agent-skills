...
## Use when
Use this workflow when you are working on go-to-market strategy and need to turn the decision into sequenced work with gates and owners. Its distinct job inside the GTM Strategy Pack is to produce the sequenced operating plan; use the neighboring skills for GTM Thesis Builder, ICP Focus Check, Channel Fit Evaluator.
...
- Domain signals to inspect: ICP, channel fit, GTM thesis, account plan, route-to-market.
...
3. Inspect the domain mechanics for this pack: ICP, channel fit, GTM thesis, account plan, route-to-market. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the sequenced operating plan, and list what should be routed to GTM Thesis Builder, ICP Focus Check, Channel Fit Evaluator.
...
- `diagnosis`: what is happening, why it matters, and which ICP signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready sequenced operating plan tied to ICP.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with GTM Thesis Builder, ICP Focus Check, Channel Fit Evaluator, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the GTM Operating Plan workflow for the following Go-To-Market Strategy situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the sequenced operating plan using the output schema, mark assumptions, and list the human review gates before anyone acts.
