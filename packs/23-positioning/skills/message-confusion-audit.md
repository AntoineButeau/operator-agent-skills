# Message Confusion Audit

## Use when
Use this workflow when you are working on positioning and need to diagnose the current state and name the operating risk. Its distinct job inside the Positioning System Pack is to produce the risk-and-evidence audit; use the neighboring skills for Category Context Map, Positioning Statement Builder, Proof Point Inventory.
...
- Domain signals to inspect: category context, positioning statement, proof point, message confusion, win/loss.
...
3. Inspect the domain mechanics for this pack: category context, positioning statement, proof point, message confusion, win/loss. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the risk-and-evidence audit, and list what should be routed to Category Context Map, Positioning Statement Builder, Proof Point Inventory.
...
- `diagnosis`: what is happening, why it matters, and which category context signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready risk-and-evidence audit tied to category context.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Category Context Map, Positioning Statement Builder, Proof Point Inventory, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Message Confusion Audit workflow for the following Positioning situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the risk-and-evidence audit using the output schema, mark assumptions, and list the human review gates before anyone acts.
