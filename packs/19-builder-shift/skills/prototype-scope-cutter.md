# Prototype Scope Cutter

## Use when
Use this workflow when you are working on the builder shift and need to handle the cutter step without overlapping the other skills in the pack. Its distinct job inside the Internal Builder Pack is to produce the prototype scope cutter; use the neighboring skills for Internal Tool Opportunity Map, Operator to Builder Spec, Builder Adoption Plan.
...
- Domain signals to inspect: builder spec, prototype scope, internal tool, adoption path, operator workflow.
...
3. Inspect the domain mechanics for this pack: builder spec, prototype scope, internal tool, adoption path, operator workflow. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the prototype scope cutter, and list what should be routed to Internal Tool Opportunity Map, Operator to Builder Spec, Builder Adoption Plan.
...
## Output
Return a prototype scope cutter with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which builder spec signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready prototype scope cutter tied to builder spec.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Internal Tool Opportunity Map, Operator to Builder Spec, Builder Adoption Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Prototype Scope Cutter workflow for the following The Builder Shift situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the prototype scope cutter using the output schema, mark assumptions, and list the human review gates before anyone acts.
