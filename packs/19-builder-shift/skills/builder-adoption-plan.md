# Builder Adoption Plan

## Use when
Use this workflow when you are working on the builder shift and need to produce a reusable artifact or operating spec. Its distinct job inside the Internal Builder Pack is to produce the sequenced operating plan; use the neighboring skills for Internal Tool Opportunity Map, Operator to Builder Spec, Prototype Scope Cutter.
...
4. Separate this skill's job from the rest of the pack: deliver the sequenced operating plan, and list what should be routed to Internal Tool Opportunity Map, Operator to Builder Spec, Prototype Scope Cutter.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready sequenced operating plan tied to builder spec.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Internal Tool Opportunity Map, Operator to Builder Spec, Prototype Scope Cutter, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Builder Adoption Plan workflow for the following The Builder Shift situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the sequenced operating plan using the output schema, mark assumptions, and list the human review gates before anyone acts.
