# Discount Signal Analysis

## Use when
Use this workflow when you are working on pricing and packaging and need to handle the analysis step without overlapping the other skills in the pack. Its distinct job inside the Pricing Decision Pack is to produce the discount signal analysis; use the neighboring skills for Pricing Segmentation Brief, Price Metric Evaluator, Pricing Change Rollout.
...
4. Separate this skill's job from the rest of the pack: deliver the discount signal analysis, and list what should be routed to Pricing Segmentation Brief, Price Metric Evaluator, Pricing Change Rollout.
...
## Output
Return a discount signal analysis with this schema:
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready discount signal analysis tied to price metric.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Pricing Segmentation Brief, Price Metric Evaluator, Pricing Change Rollout, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Discount Signal Analysis workflow for the following Pricing and Packaging situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the discount signal analysis using the output schema, mark assumptions, and list the human review gates before anyone acts.
