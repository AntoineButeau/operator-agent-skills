# Pricing Segmentation Brief

## Use when
Use this workflow when you are working on pricing and packaging and need to compress the situation into a decision-ready brief. Its distinct job inside the Pricing Decision Pack is to produce the operator memo; use the neighboring skills for Price Metric Evaluator, Discount Signal Analysis, Pricing Change Rollout.
...
4. Separate this skill's job from the rest of the pack: deliver the operator memo, and list what should be routed to Price Metric Evaluator, Discount Signal Analysis, Pricing Change Rollout.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready operator memo tied to price metric.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Price Metric Evaluator, Discount Signal Analysis, Pricing Change Rollout, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Pricing Segmentation Brief workflow for the following Pricing and Packaging situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the operator memo using the output schema, mark assumptions, and list the human review gates before anyone acts.
