...
## Use when
Use this workflow when you are working on pricing and packaging and need to turn the decision into sequenced work with gates and owners. Its distinct job inside the Pricing Decision Pack is to produce the sequenced operating plan; use the neighboring skills for Pricing Segmentation Brief, Price Metric Evaluator, Discount Signal Analysis.
...
4. Separate this skill's job from the rest of the pack: deliver the sequenced operating plan, and list what should be routed to Pricing Segmentation Brief, Price Metric Evaluator, Discount Signal Analysis.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready sequenced operating plan tied to price metric.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Pricing Segmentation Brief, Price Metric Evaluator, Discount Signal Analysis, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Pricing Change Rollout workflow for the following Pricing and Packaging situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the sequenced operating plan using the output schema, mark assumptions, and list the human review gates before anyone acts.
