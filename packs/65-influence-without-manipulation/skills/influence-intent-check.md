# Influence Intent Check

## Use when
Use this workflow when you are working on Influence Without Manipulation and need to handle the system step without overlapping the other skills in the pack. Its distinct job inside the Ethical Influence Pack is to produce the influence intent check; use the neighboring skills for Stakeholder Motivation Map, Ethical Persuasion Brief, Manipulation Risk Review.
...
- Domain signals to inspect: desired action, mutual benefit, agency, transparency, pressure risk.
...
3. Inspect the domain mechanics for this pack: desired action, mutual benefit, agency, transparency, pressure risk. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the influence intent check, and list what should be routed to Stakeholder Motivation Map, Ethical Persuasion Brief, Manipulation Risk Review.
...
## Output
Return a influence intent check with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which desired action signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready influence intent check tied to desired action.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Stakeholder Motivation Map, Ethical Persuasion Brief, Manipulation Risk Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Influence Intent Check workflow for the following Influence Without Manipulation situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the influence intent check using the output schema, mark assumptions, and list the human review gates before anyone acts.
