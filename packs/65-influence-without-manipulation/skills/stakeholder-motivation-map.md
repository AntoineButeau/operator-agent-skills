...
## Use when
Use this workflow when you are working on Influence Without Manipulation and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the Ethical Influence Pack is to produce the motivation map; use the neighboring skills for Influence Intent Check, Ethical Persuasion Brief, Manipulation Risk Review.
...
- Domain signals to inspect: interest, constraint, status concern, incentive, timing.
...
3. Inspect the domain mechanics for this pack: interest, constraint, status concern, incentive, timing. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the motivation map, and list what should be routed to Influence Intent Check, Ethical Persuasion Brief, Manipulation Risk Review.
...
## Output
Return a motivation map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which interest signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready motivation map tied to interest.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Influence Intent Check, Ethical Persuasion Brief, Manipulation Risk Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Stakeholder Motivation Map workflow for the following Influence Without Manipulation situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the motivation map using the output schema, mark assumptions, and list the human review gates before anyone acts.
