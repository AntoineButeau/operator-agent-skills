# Build Buy Partner Brief

## Use when
Use this workflow when you are working on the full-stack company: vertical integration in the ai era and need to compress the situation into a decision-ready brief. Its distinct job inside the Vertical Integration Pack is to produce the operator memo; use the neighboring skills for Integration Boundary Audit, Control Point Map, Vertical Integration Roadmap.
...
- Domain signals to inspect: control point, build-buy-partner, integration boundary, vertical integration, operating dependency.
...
3. Inspect the domain mechanics for this pack: control point, build-buy-partner, integration boundary, vertical integration, operating dependency. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the operator memo, and list what should be routed to Integration Boundary Audit, Control Point Map, Vertical Integration Roadmap.
...
- `diagnosis`: what is happening, why it matters, and which control point signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready operator memo tied to control point.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Integration Boundary Audit, Control Point Map, Vertical Integration Roadmap, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Build Buy Partner Brief workflow for the following The Full-Stack Company: Vertical Integration in the AI Era situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the operator memo using the output schema, mark assumptions, and list the human review gates before anyone acts.
