# Announcement to Adoption Plan

## Use when
Use this workflow when you are working on change that actually takes: how companies move from announcement to adoption and need to turn the decision into sequenced work with gates and owners. Its distinct job inside the Change Adoption Pack is to produce the sequenced operating plan; use the neighboring skills for Change Readiness Scan, Resistance Diagnosis, Change Embedding Review.
...
- Domain signals to inspect: change load, adoption path, resistance signal, embedding review, manager cascade.
...
3. Inspect the domain mechanics for this pack: change load, adoption path, resistance signal, embedding review, manager cascade. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the sequenced operating plan, and list what should be routed to Change Readiness Scan, Resistance Diagnosis, Change Embedding Review.
...
- `diagnosis`: what is happening, why it matters, and which change load signal changed.
