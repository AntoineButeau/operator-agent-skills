...
## Use when
Use this workflow when you are working on The Forward-Deployed Company and need to diagnose the current state and name the operating risk. Its distinct job inside the Forward-Deployed Company Pack is to produce the risk review; use the neighboring skills for Field Operating Model Map, Embedded Team Scope Brief, Customer Feedback Loop Design.
...
- Domain signals to inspect: customer commitment, implementation risk, support load, knowledge transfer, commercial exposure.
...
3. Inspect the domain mechanics for this pack: customer commitment, implementation risk, support load, knowledge transfer, commercial exposure. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the risk review, and list what should be routed to Field Operating Model Map, Embedded Team Scope Brief, Customer Feedback Loop Design.
...
## Output
Return a risk review with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which customer commitment signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready risk review tied to customer commitment.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Field Operating Model Map, Embedded Team Scope Brief, Customer Feedback Loop Design, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Deployment Risk Review workflow for the following The Forward-Deployed Company situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the risk review using the output schema, mark assumptions, and list the human review gates before anyone acts.
