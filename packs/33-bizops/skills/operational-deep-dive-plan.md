# Operational Deep Dive Plan

## Use when
Use this workflow when you are working on bizops / business operations and need to turn the decision into sequenced work with gates and owners. Its distinct job inside the BizOps Decision Support Pack is to produce the sequenced operating plan; use the neighboring skills for BizOps Problem Framing, Decision Support Model Spec, Recommendation Memo Builder.
...
- Domain signals to inspect: problem framing, decision model, deep dive plan, recommendation memo, operating metric.
...
3. Inspect the domain mechanics for this pack: problem framing, decision model, deep dive plan, recommendation memo, operating metric. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the sequenced operating plan, and list what should be routed to BizOps Problem Framing, Decision Support Model Spec, Recommendation Memo Builder.
...
- `diagnosis`: what is happening, why it matters, and which problem framing signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready sequenced operating plan tied to problem framing.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with BizOps Problem Framing, Decision Support Model Spec, Recommendation Memo Builder, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Operational Deep Dive Plan workflow for the following BizOps / Business Operations situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the sequenced operating plan using the output schema, mark assumptions, and list the human review gates before anyone acts.
