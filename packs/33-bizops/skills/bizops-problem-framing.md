# BizOps Problem Framing

## Use when
Use this workflow when you are working on bizops / business operations and need to handle the framing step without overlapping the other skills in the pack. Its distinct job inside the BizOps Decision Support Pack is to produce the bizops problem framing; use the neighboring skills for Decision Support Model Spec, Operational Deep Dive Plan, Recommendation Memo Builder.
...
4. Separate this skill's job from the rest of the pack: deliver the bizops problem framing, and list what should be routed to Decision Support Model Spec, Operational Deep Dive Plan, Recommendation Memo Builder.
...
## Output
Return a bizops problem framing with this schema:
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready bizops problem framing tied to problem framing.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Decision Support Model Spec, Operational Deep Dive Plan, Recommendation Memo Builder, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the BizOps Problem Framing workflow for the following BizOps / Business Operations situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the bizops problem framing using the output schema, mark assumptions, and list the human review gates before anyone acts.
