# Recommendation Memo Builder

## Use when
Use this workflow when you are working on bizops / business operations and need to produce a reusable artifact or operating spec. Its distinct job inside the BizOps Decision Support Pack is to produce the operator memo; use the neighboring skills for BizOps Problem Framing, Decision Support Model Spec, Operational Deep Dive Plan.
...
4. Separate this skill's job from the rest of the pack: deliver the operator memo, and list what should be routed to BizOps Problem Framing, Decision Support Model Spec, Operational Deep Dive Plan.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready operator memo tied to problem framing.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with BizOps Problem Framing, Decision Support Model Spec, Operational Deep Dive Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Recommendation Memo Builder workflow for the following BizOps / Business Operations situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the operator memo using the output schema, mark assumptions, and list the human review gates before anyone acts.
