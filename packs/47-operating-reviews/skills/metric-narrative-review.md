# Metric Narrative Review

## Use when
Use this workflow when you are working on Operating Reviews That Actually Drive Decisions and need to diagnose the current state and name the operating risk. Its distinct job inside the Operating Review Pack is to produce the metric narrative review; use the neighboring skills for Operating Review Agenda Builder, Decision Followthrough Audit, Review Cadence Redesign.
...
- Domain signals to inspect: metric movement, causal story, assumption, counter-signal, decision implication.
...
3. Inspect the domain mechanics for this pack: metric movement, causal story, assumption, counter-signal, decision implication. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the metric narrative review, and list what should be routed to Operating Review Agenda Builder, Decision Followthrough Audit, Review Cadence Redesign.
...
## Output
Return a metric narrative review with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which metric movement signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready metric narrative review tied to metric movement.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Operating Review Agenda Builder, Decision Followthrough Audit, Review Cadence Redesign, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Metric Narrative Review workflow for the following Operating Reviews That Actually Drive Decisions situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the metric narrative review using the output schema, mark assumptions, and list the human review gates before anyone acts.
