# Operating Review Agenda Builder

## Use when
Use this workflow when you are working on Operating Reviews That Actually Drive Decisions and need to diagnose the current state and name the operating risk. Its distinct job inside the Operating Review Pack is to produce the review agenda; use the neighboring skills for Decision Followthrough Audit, Metric Narrative Review, Review Cadence Redesign.
...
- Domain signals to inspect: decision agenda, metric owner, variance, action log, escalation.
...
3. Inspect the domain mechanics for this pack: decision agenda, metric owner, variance, action log, escalation. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the review agenda, and list what should be routed to Decision Followthrough Audit, Metric Narrative Review, Review Cadence Redesign.
...
## Output
Return a review agenda with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which decision agenda signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready review agenda tied to decision agenda.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Decision Followthrough Audit, Metric Narrative Review, Review Cadence Redesign, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Operating Review Agenda Builder workflow for the following Operating Reviews That Actually Drive Decisions situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the review agenda using the output schema, mark assumptions, and list the human review gates before anyone acts.
