# AI Adoption Readiness Map

## Use when
Use this workflow when you are working on AI Adoption That Actually Changes the Company and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the AI Adoption Pack is to produce the adoption readiness map; use the neighboring skills for Behavior Change Plan, Adoption Friction Diagnosis, Adoption Measurement System.
...
- Domain signals to inspect: workflow fit, manager behavior, tool access, training gap, incentive.
...
3. Inspect the domain mechanics for this pack: workflow fit, manager behavior, tool access, training gap, incentive. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the adoption readiness map, and list what should be routed to Behavior Change Plan, Adoption Friction Diagnosis, Adoption Measurement System.
...
## Output
Return a adoption readiness map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which workflow fit signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready adoption readiness map tied to workflow fit.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Behavior Change Plan, Adoption Friction Diagnosis, Adoption Measurement System, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the AI Adoption Readiness Map workflow for the following AI Adoption That Actually Changes the Company situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the adoption readiness map using the output schema, mark assumptions, and list the human review gates before anyone acts.
