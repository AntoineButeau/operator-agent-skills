...
## Use when
Use this workflow when you are working on AI Adoption That Actually Changes the Company and need to define the repeatable operating system and cadence. Its distinct job inside the AI Adoption Pack is to produce the measurement system; use the neighboring skills for AI Adoption Readiness Map, Behavior Change Plan, Adoption Friction Diagnosis.
...
- Domain signals to inspect: usage quality, outcome metric, cohort, leading indicator, review cadence.
...
3. Inspect the domain mechanics for this pack: usage quality, outcome metric, cohort, leading indicator, review cadence. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the measurement system, and list what should be routed to AI Adoption Readiness Map, Behavior Change Plan, Adoption Friction Diagnosis.
...
## Output
Return a measurement system with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which usage quality signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready measurement system tied to usage quality.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with AI Adoption Readiness Map, Behavior Change Plan, Adoption Friction Diagnosis, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Adoption Measurement System workflow for the following AI Adoption That Actually Changes the Company situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the measurement system using the output schema, mark assumptions, and list the human review gates before anyone acts.
