# Assumption Refresh Loop

## Use when
Use this workflow when you are working on how to be a live player and need to handle the loop step without overlapping the other skills in the pack. Its distinct job inside the Reality Contact Pack is to produce the assumption refresh loop; use the neighboring skills for Reality Contact Audit, Live Option Generator, Field Signal Review.
...
4. Separate this skill's job from the rest of the pack: deliver the assumption refresh loop, and list what should be routed to Reality Contact Audit, Live Option Generator, Field Signal Review.
...
## Output
Return a assumption refresh loop with this schema:
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready assumption refresh loop tied to field signal.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Reality Contact Audit, Live Option Generator, Field Signal Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Assumption Refresh Loop workflow for the following How to Be a Live Player situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the assumption refresh loop using the output schema, mark assumptions, and list the human review gates before anyone acts.
