...
## Use when
Use this workflow when you are working on founder mode that actually scales and need to handle the capture step without overlapping the other skills in the pack. Its distinct job inside the Founder Operating Mode Pack is to produce the founder context capture; use the neighboring skills for Founder Bottleneck Audit, Founder Decision Rights Reset, Scaling Founder Rituals.
...
4. Separate this skill's job from the rest of the pack: deliver the founder context capture, and list what should be routed to Founder Bottleneck Audit, Founder Decision Rights Reset, Scaling Founder Rituals.
...
## Output
Return a founder context capture with this schema:
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready founder context capture tied to founder context.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Founder Bottleneck Audit, Founder Decision Rights Reset, Scaling Founder Rituals, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Founder Context Capture workflow for the following Founder Mode That Actually Scales situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the founder context capture using the output schema, mark assumptions, and list the human review gates before anyone acts.
