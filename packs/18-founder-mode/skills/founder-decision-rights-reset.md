# Founder Decision Rights Reset

## Use when
Use this workflow when you are working on founder mode that actually scales and need to handle the reset step without overlapping the other skills in the pack. Its distinct job inside the Founder Operating Mode Pack is to produce the founder decision rights reset; use the neighboring skills for Founder Bottleneck Audit, Founder Context Capture, Scaling Founder Rituals.
...
- Domain signals to inspect: founder context, founder decision rights, constraint relief point, scaling ritual, context capture.
...
3. Inspect the domain mechanics for this pack: founder context, founder decision rights, constraint relief point, scaling ritual, context capture. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the founder decision rights reset, and list what should be routed to Founder Bottleneck Audit, Founder Context Capture, Scaling Founder Rituals.
...
## Output
Return a founder decision rights reset with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which founder context signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready founder decision rights reset tied to founder context.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Founder Bottleneck Audit, Founder Context Capture, Scaling Founder Rituals, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Founder Decision Rights Reset workflow for the following Founder Mode That Actually Scales situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the founder decision rights reset using the output schema, mark assumptions, and list the human review gates before anyone acts.
