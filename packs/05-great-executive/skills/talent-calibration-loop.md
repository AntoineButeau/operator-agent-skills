# Talent Calibration Loop

## Use when
Use this workflow when you are working on being a great executive and need to handle the loop step without overlapping the other skills in the pack. Its distinct job inside the Executive Attention Pack is to produce the talent calibration loop; use the neighboring skills for Executive Attention Audit, Strategic No Brief, Legacy System Review.
...
- Domain signals to inspect: executive attention, strategic no, talent calibration, legacy system, decision rights.
...
3. Inspect the domain mechanics for this pack: executive attention, strategic no, talent calibration, legacy system, decision rights. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the talent calibration loop, and list what should be routed to Executive Attention Audit, Strategic No Brief, Legacy System Review.
...
## Output
Return a talent calibration loop with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which executive attention signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready talent calibration loop tied to executive attention.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Executive Attention Audit, Strategic No Brief, Legacy System Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Talent Calibration Loop workflow for the following Being a Great Executive situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the talent calibration loop using the output schema, mark assumptions, and list the human review gates before anyone acts.
