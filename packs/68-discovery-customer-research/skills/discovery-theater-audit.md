# Discovery Theater Audit

## Use when
Use this workflow when you are working on Discovery and Customer Research Without Theater and need to find gaps between stated process and observed behavior. Its distinct job inside the Discovery Research Pack is to produce the theater audit; use the neighboring skills for Research Question Framing, Customer Signal Synthesis, Research to Decision Brief.
...
- Domain signals to inspect: performative research, confirmation bias, weak sample, decision avoidance, insight quality.
...
3. Inspect the domain mechanics for this pack: performative research, confirmation bias, weak sample, decision avoidance, insight quality. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the theater audit, and list what should be routed to Research Question Framing, Customer Signal Synthesis, Research to Decision Brief.
...
## Output
Return a theater audit with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which performative research signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready theater audit tied to performative research.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Research Question Framing, Customer Signal Synthesis, Research to Decision Brief, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Discovery Theater Audit workflow for the following Discovery and Customer Research Without Theater situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the theater audit using the output schema, mark assumptions, and list the human review gates before anyone acts.
