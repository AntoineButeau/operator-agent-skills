# Research to Decision Brief

## Use when
Use this workflow when you are working on Discovery and Customer Research Without Theater and need to produce a reusable decision brief or operating spec. Its distinct job inside the Discovery Research Pack is to produce the decision brief; use the neighboring skills for Research Question Framing, Customer Signal Synthesis, Discovery Theater Audit.
...
- Domain signals to inspect: finding, confidence, product implication, tradeoff, next experiment.
...
3. Inspect the domain mechanics for this pack: finding, confidence, product implication, tradeoff, next experiment. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the decision brief, and list what should be routed to Research Question Framing, Customer Signal Synthesis, Discovery Theater Audit.
...
## Output
Return a decision brief with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which finding signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready decision brief tied to finding.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Research Question Framing, Customer Signal Synthesis, Discovery Theater Audit, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Research to Decision Brief workflow for the following Discovery and Customer Research Without Theater situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the decision brief using the output schema, mark assumptions, and list the human review gates before anyone acts.
