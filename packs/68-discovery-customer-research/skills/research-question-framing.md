...
## Use when
Use this workflow when you are working on Discovery and Customer Research Without Theater and need to handle the system step without overlapping the other skills in the pack. Its distinct job inside the Discovery Research Pack is to produce the research question framing; use the neighboring skills for Customer Signal Synthesis, Discovery Theater Audit, Research to Decision Brief.
...
- Domain signals to inspect: learning goal, decision dependency, assumption, customer segment, evidence standard.
...
3. Inspect the domain mechanics for this pack: learning goal, decision dependency, assumption, customer segment, evidence standard. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the research question framing, and list what should be routed to Customer Signal Synthesis, Discovery Theater Audit, Research to Decision Brief.
...
## Output
Return a research question framing with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which learning goal signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready research question framing tied to learning goal.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Customer Signal Synthesis, Discovery Theater Audit, Research to Decision Brief, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Research Question Framing workflow for the following Discovery and Customer Research Without Theater situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the research question framing using the output schema, mark assumptions, and list the human review gates before anyone acts.
