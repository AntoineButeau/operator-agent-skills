...
## Use when
Use this workflow when you are working on The AI Control Plane and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the AI Control Plane Pack is to produce the agent registry map; use the neighboring skills for Permission Boundary Review, Control Plane Operating Policy, Agent Incident Retrospective.
...
- Domain signals to inspect: agent owner, permission, workflow, audit trail, context source.
...
3. Inspect the domain mechanics for this pack: agent owner, permission, workflow, audit trail, context source. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the agent registry map, and list what should be routed to Permission Boundary Review, Control Plane Operating Policy, Agent Incident Retrospective.
...
## Output
Return a agent registry map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which agent owner signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready agent registry map tied to agent owner.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Permission Boundary Review, Control Plane Operating Policy, Agent Incident Retrospective, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Agent Registry Map workflow for the following The AI Control Plane situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the agent registry map using the output schema, mark assumptions, and list the human review gates before anyone acts.
