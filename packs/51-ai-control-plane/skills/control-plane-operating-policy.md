# Control Plane Operating Policy

## Use when
Use this workflow when you are working on The AI Control Plane and need to handle the system step without overlapping the other skills in the pack. Its distinct job inside the AI Control Plane Pack is to produce the operating policy; use the neighboring skills for Agent Registry Map, Permission Boundary Review, Agent Incident Retrospective.
...
- Domain signals to inspect: agent lifecycle, approval workflow, monitoring, incident response, governance forum.
...
3. Inspect the domain mechanics for this pack: agent lifecycle, approval workflow, monitoring, incident response, governance forum. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the operating policy, and list what should be routed to Agent Registry Map, Permission Boundary Review, Agent Incident Retrospective.
...
## Output
Return a operating policy with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which agent lifecycle signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready operating policy tied to agent lifecycle.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Agent Registry Map, Permission Boundary Review, Agent Incident Retrospective, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Control Plane Operating Policy workflow for the following The AI Control Plane situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the operating policy using the output schema, mark assumptions, and list the human review gates before anyone acts.
