...
## Use when
Use this workflow when you are working on The AI Control Plane and need to diagnose the current state and name the operating risk. Its distinct job inside the AI Control Plane Pack is to produce the permission boundary review; use the neighboring skills for Agent Registry Map, Control Plane Operating Policy, Agent Incident Retrospective.
...
- Domain signals to inspect: tool access, data sensitivity, action scope, escalation rule, revocation path.
...
3. Inspect the domain mechanics for this pack: tool access, data sensitivity, action scope, escalation rule, revocation path. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the permission boundary review, and list what should be routed to Agent Registry Map, Control Plane Operating Policy, Agent Incident Retrospective.
...
## Output
Return a permission boundary review with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which tool access signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready permission boundary review tied to tool access.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Agent Registry Map, Control Plane Operating Policy, Agent Incident Retrospective, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Permission Boundary Review workflow for the following The AI Control Plane situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the permission boundary review using the output schema, mark assumptions, and list the human review gates before anyone acts.
