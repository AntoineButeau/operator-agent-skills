...
## Use when
Use this workflow when you are working on The AI Control Plane and need to extract lessons and update the operating system. Its distinct job inside the AI Control Plane Pack is to produce the incident retrospective; use the neighboring skills for Agent Registry Map, Permission Boundary Review, Control Plane Operating Policy.
...
- Domain signals to inspect: agent action, failure mode, detection gap, rollback, control improvement.
...
3. Inspect the domain mechanics for this pack: agent action, failure mode, detection gap, rollback, control improvement. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the incident retrospective, and list what should be routed to Agent Registry Map, Permission Boundary Review, Control Plane Operating Policy.
...
## Output
Return a incident retrospective with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which agent action signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready incident retrospective tied to agent action.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Agent Registry Map, Permission Boundary Review, Control Plane Operating Policy, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Agent Incident Retrospective workflow for the following The AI Control Plane situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the incident retrospective using the output schema, mark assumptions, and list the human review gates before anyone acts.
