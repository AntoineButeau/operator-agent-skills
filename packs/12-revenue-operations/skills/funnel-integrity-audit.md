...
## Use when
Use this workflow when you are working on revenue operations and need to diagnose the current state and name the operating risk. Its distinct job inside the RevOps System Pack is to produce the risk-and-evidence audit; use the neighboring skills for GTM Handoff Map, Forecast Reliability Review, RevOps Change Control.
...
4. Separate this skill's job from the rest of the pack: deliver the risk-and-evidence audit, and list what should be routed to GTM Handoff Map, Forecast Reliability Review, RevOps Change Control.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready risk-and-evidence audit tied to forecast commit.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with GTM Handoff Map, Forecast Reliability Review, RevOps Change Control, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Funnel Integrity Audit workflow for the following Revenue Operations situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the risk-and-evidence audit using the output schema, mark assumptions, and list the human review gates before anyone acts.
