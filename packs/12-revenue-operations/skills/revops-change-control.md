# RevOps Change Control

## Use when
Use this workflow when you are working on revenue operations and need to handle the control step without overlapping the other skills in the pack. Its distinct job inside the RevOps System Pack is to produce the revops change control; use the neighboring skills for Funnel Integrity Audit, GTM Handoff Map, Forecast Reliability Review.
...
- Domain signals to inspect: forecast commit, pipeline inspection, stage hygiene, GTM handoff, change control.
...
3. Inspect the domain mechanics for this pack: forecast commit, pipeline inspection, stage hygiene, GTM handoff, change control. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the revops change control, and list what should be routed to Funnel Integrity Audit, GTM Handoff Map, Forecast Reliability Review.
...
## Output
Return a revops change control with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which forecast commit signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready revops change control tied to forecast commit.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Funnel Integrity Audit, GTM Handoff Map, Forecast Reliability Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the RevOps Change Control workflow for the following Revenue Operations situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the revops change control using the output schema, mark assumptions, and list the human review gates before anyone acts.
