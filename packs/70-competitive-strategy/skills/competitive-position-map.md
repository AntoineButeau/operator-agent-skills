...
## Use when
Use this workflow when you are working on Competitive Strategy and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the Competitive Strategy Pack is to produce the position map; use the neighboring skills for Moat Evidence Review, Competitive Response Brief, Strategy War Game.
...
- Domain signals to inspect: competitor move, customer criterion, wedge, differentiation, vulnerability.
...
3. Inspect the domain mechanics for this pack: competitor move, customer criterion, wedge, differentiation, vulnerability. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the position map, and list what should be routed to Moat Evidence Review, Competitive Response Brief, Strategy War Game.
...
## Output
Return a position map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which competitor move signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready position map tied to competitor move.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Moat Evidence Review, Competitive Response Brief, Strategy War Game, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Competitive Position Map workflow for the following Competitive Strategy situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the position map using the output schema, mark assumptions, and list the human review gates before anyone acts.
