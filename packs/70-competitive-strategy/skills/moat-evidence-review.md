# Moat Evidence Review

## Use when
Use this workflow when you are working on Competitive Strategy and need to diagnose the current state and name the operating risk. Its distinct job inside the Competitive Strategy Pack is to produce the moat evidence review; use the neighboring skills for Competitive Position Map, Competitive Response Brief, Strategy War Game.
...
- Domain signals to inspect: switching cost, data advantage, brand, distribution, execution speed.
...
3. Inspect the domain mechanics for this pack: switching cost, data advantage, brand, distribution, execution speed. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the moat evidence review, and list what should be routed to Competitive Position Map, Competitive Response Brief, Strategy War Game.
...
## Output
Return a moat evidence review with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which switching cost signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready moat evidence review tied to switching cost.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Competitive Position Map, Competitive Response Brief, Strategy War Game, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Moat Evidence Review workflow for the following Competitive Strategy situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the moat evidence review using the output schema, mark assumptions, and list the human review gates before anyone acts.
