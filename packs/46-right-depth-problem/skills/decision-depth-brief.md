...
## Use when
Use this workflow when you are working on The Right Depth for the Problem and need to produce a reusable decision brief or operating spec. Its distinct job inside the Right Depth Pack is to produce the decision depth brief; use the neighboring skills for Depth Selection Check, Analysis Scope Cutter, Precision Risk Review.
...
- Domain signals to inspect: depth choice, evidence standard, confidence level, owner judgment, review trigger.
...
3. Inspect the domain mechanics for this pack: depth choice, evidence standard, confidence level, owner judgment, review trigger. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the decision depth brief, and list what should be routed to Depth Selection Check, Analysis Scope Cutter, Precision Risk Review.
...
## Output
Return a decision depth brief with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which depth choice signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready decision depth brief tied to depth choice.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Depth Selection Check, Analysis Scope Cutter, Precision Risk Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Decision Depth Brief workflow for the following The Right Depth for the Problem situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the decision depth brief using the output schema, mark assumptions, and list the human review gates before anyone acts.
