# Depth Selection Check

## Use when
Use this workflow when you are working on The Right Depth for the Problem and need to handle the system step without overlapping the other skills in the pack. Its distinct job inside the Right Depth Pack is to produce the depth selection check; use the neighboring skills for Analysis Scope Cutter, Precision Risk Review, Decision Depth Brief.
...
- Domain signals to inspect: problem depth, reversibility, uncertainty, decision cost, timebox.
...
3. Inspect the domain mechanics for this pack: problem depth, reversibility, uncertainty, decision cost, timebox. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the depth selection check, and list what should be routed to Analysis Scope Cutter, Precision Risk Review, Decision Depth Brief.
...
## Output
Return a depth selection check with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which problem depth signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready depth selection check tied to problem depth.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Analysis Scope Cutter, Precision Risk Review, Decision Depth Brief, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Depth Selection Check workflow for the following The Right Depth for the Problem situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the depth selection check using the output schema, mark assumptions, and list the human review gates before anyone acts.
