...
## Use when
Use this workflow when you are working on organizational design and need to handle the matrix step without overlapping the other skills in the pack. Its distinct job inside the Org Design Diagnostic Pack is to produce the owner-and-flow map; use the neighboring skills for Org Design Diagnosis, Handoff Friction Map, Role Clarity Redesign.
...
4. Separate this skill's job from the rest of the pack: deliver the owner-and-flow map, and list what should be routed to Org Design Diagnosis, Handoff Friction Map, Role Clarity Redesign.
...
- Blurring this skill with Org Design Diagnosis, Handoff Friction Map, Role Clarity Redesign, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Decision Rights Matrix workflow for the following Organizational Design situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the owner-and-flow map using the output schema, mark assumptions, and list the human review gates before anyone acts.
