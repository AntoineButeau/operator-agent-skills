# Role Clarity Redesign

## Use when
Use this workflow when you are working on organizational design and need to produce a reusable artifact or operating spec. Its distinct job inside the Org Design Diagnostic Pack is to produce the role clarity redesign; use the neighboring skills for Org Design Diagnosis, Decision Rights Matrix, Handoff Friction Map.
...
- Domain signals to inspect: role charter, decision rights, handoff map, org seam, accountability boundary.
...
3. Inspect the domain mechanics for this pack: role charter, decision rights, handoff map, org seam, accountability boundary. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the role clarity redesign, and list what should be routed to Org Design Diagnosis, Decision Rights Matrix, Handoff Friction Map.
...
## Output
Return a role clarity redesign with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which role charter signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready role clarity redesign tied to role charter.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Org Design Diagnosis, Decision Rights Matrix, Handoff Friction Map, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Role Clarity Redesign workflow for the following Organizational Design situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the role clarity redesign using the output schema, mark assumptions, and list the human review gates before anyone acts.
