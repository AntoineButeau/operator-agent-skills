...
## Use when
Use this workflow when you are working on building ai products and need to turn the decision into sequenced work with gates and owners. Its distinct job inside the AI Product Pack is to produce the sequenced operating plan; use the neighboring skills for AI Use Case Qualification, Model Behavior Evaluation Plan, AI Product Risk Review.
...
- Domain signals to inspect: model behavior, evaluation set, AI launch gate, risk review, use-case qualification.
...
3. Inspect the domain mechanics for this pack: model behavior, evaluation set, AI launch gate, risk review, use-case qualification. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the sequenced operating plan, and list what should be routed to AI Use Case Qualification, Model Behavior Evaluation Plan, AI Product Risk Review.
...
- `diagnosis`: what is happening, why it matters, and which model behavior signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready sequenced operating plan tied to model behavior.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with AI Use Case Qualification, Model Behavior Evaluation Plan, AI Product Risk Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the AI Feature Launch Plan workflow for the following Building AI Products situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the sequenced operating plan using the output schema, mark assumptions, and list the human review gates before anyone acts.
