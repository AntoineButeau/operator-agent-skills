...
## Use when
Use this workflow when you are working on The AI-Native Security Model and need to diagnose the current state and name the operating risk. Its distinct job inside the AI-Native Security Pack is to produce the permission review; use the neighboring skills for Agent Threat Model, AI Security Control Map, Red-Team Findings Brief.
...
- Domain signals to inspect: sensitive data, least privilege, retention, audit trail, policy boundary.
...
3. Inspect the domain mechanics for this pack: sensitive data, least privilege, retention, audit trail, policy boundary. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the permission review, and list what should be routed to Agent Threat Model, AI Security Control Map, Red-Team Findings Brief.
...
## Output
Return a permission review with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which sensitive data signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready permission review tied to sensitive data.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Agent Threat Model, AI Security Control Map, Red-Team Findings Brief, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Data Permission Review workflow for the following The AI-Native Security Model situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the permission review using the output schema, mark assumptions, and list the human review gates before anyone acts.
