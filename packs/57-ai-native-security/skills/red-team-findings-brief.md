# Red-Team Findings Brief

## Use when
Use this workflow when you are working on The AI-Native Security Model and need to produce a reusable decision brief or operating spec. Its distinct job inside the AI-Native Security Pack is to produce the findings brief; use the neighboring skills for Agent Threat Model, Data Permission Review, AI Security Control Map.
...
- Domain signals to inspect: attack path, exploit condition, impact, mitigation, residual risk.
...
3. Inspect the domain mechanics for this pack: attack path, exploit condition, impact, mitigation, residual risk. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the findings brief, and list what should be routed to Agent Threat Model, Data Permission Review, AI Security Control Map.
...
## Output
Return a findings brief with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which attack path signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready findings brief tied to attack path.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Agent Threat Model, Data Permission Review, AI Security Control Map, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Red-Team Findings Brief workflow for the following The AI-Native Security Model situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the findings brief using the output schema, mark assumptions, and list the human review gates before anyone acts.
