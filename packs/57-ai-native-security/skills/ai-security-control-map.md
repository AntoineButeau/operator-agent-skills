...
## Use when
Use this workflow when you are working on The AI-Native Security Model and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the AI-Native Security Pack is to produce the control map; use the neighboring skills for Agent Threat Model, Data Permission Review, Red-Team Findings Brief.
...
- Domain signals to inspect: preventive control, detective control, human approval, sandbox, incident path.
...
3. Inspect the domain mechanics for this pack: preventive control, detective control, human approval, sandbox, incident path. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the control map, and list what should be routed to Agent Threat Model, Data Permission Review, Red-Team Findings Brief.
...
## Output
Return a control map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which preventive control signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready control map tied to preventive control.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Agent Threat Model, Data Permission Review, Red-Team Findings Brief, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the AI Security Control Map workflow for the following The AI-Native Security Model situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the control map using the output schema, mark assumptions, and list the human review gates before anyone acts.
