...
## Use when
Use this workflow when you are working on The AI-Native Security Model and need to handle the system step without overlapping the other skills in the pack. Its distinct job inside the AI-Native Security Pack is to produce the threat model; use the neighboring skills for Data Permission Review, AI Security Control Map, Red-Team Findings Brief.
...
- Domain signals to inspect: prompt injection, tool misuse, data exfiltration, authorization gap, supply chain.
...
3. Inspect the domain mechanics for this pack: prompt injection, tool misuse, data exfiltration, authorization gap, supply chain. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the threat model, and list what should be routed to Data Permission Review, AI Security Control Map, Red-Team Findings Brief.
...
## Output
Return a threat model with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which prompt injection signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready threat model tied to prompt injection.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Data Permission Review, AI Security Control Map, Red-Team Findings Brief, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Agent Threat Model workflow for the following The AI-Native Security Model situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the threat model using the output schema, mark assumptions, and list the human review gates before anyone acts.
