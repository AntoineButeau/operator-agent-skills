...
## Use when
Use this workflow when you are working on technical literacy / technical taste and need to compress the situation into a decision-ready brief. Its distinct job inside the Technical Taste Pack is to produce the operator memo; use the neighboring skills for Architecture Risk Scan, Technical Taste Review, AI Code Review Context Pack.
...
- Domain signals to inspect: architecture risk, code review context, technical decision, taste tradeoff, maintainability.
...
3. Inspect the domain mechanics for this pack: architecture risk, code review context, technical decision, taste tradeoff, maintainability. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the operator memo, and list what should be routed to Architecture Risk Scan, Technical Taste Review, AI Code Review Context Pack.
...
- `diagnosis`: what is happening, why it matters, and which architecture risk signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready operator memo tied to architecture risk.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Architecture Risk Scan, Technical Taste Review, AI Code Review Context Pack, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Technical Decision Brief workflow for the following Technical Literacy / Technical Taste situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the operator memo using the output schema, mark assumptions, and list the human review gates before anyone acts.
