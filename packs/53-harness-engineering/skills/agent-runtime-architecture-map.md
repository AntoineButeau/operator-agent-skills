# Agent Runtime Architecture Map

## Use when
Use this workflow when you are working on Harness Engineering: The Runtime Layer Around AI Agents and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the Harness Engineering Pack is to produce the runtime architecture map; use the neighboring skills for Eval Harness Spec, Context Window Management Plan, Runtime Failure Mode Review.
...
- Domain signals to inspect: orchestration, memory, tool call, sandbox, evaluation hook.
...
3. Inspect the domain mechanics for this pack: orchestration, memory, tool call, sandbox, evaluation hook. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the runtime architecture map, and list what should be routed to Eval Harness Spec, Context Window Management Plan, Runtime Failure Mode Review.
...
## Output
Return a runtime architecture map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which orchestration signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready runtime architecture map tied to orchestration.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Eval Harness Spec, Context Window Management Plan, Runtime Failure Mode Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Agent Runtime Architecture Map workflow for the following Harness Engineering: The Runtime Layer Around AI Agents situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the runtime architecture map using the output schema, mark assumptions, and list the human review gates before anyone acts.
