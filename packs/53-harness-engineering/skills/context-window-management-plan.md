...
## Use when
Use this workflow when you are working on Harness Engineering: The Runtime Layer Around AI Agents and need to turn the diagnosis into an executable plan with owners and gates. Its distinct job inside the Harness Engineering Pack is to produce the context management plan; use the neighboring skills for Agent Runtime Architecture Map, Eval Harness Spec, Runtime Failure Mode Review.
...
- Domain signals to inspect: context pack, truncation risk, retrieval rule, summary boundary, freshness.
...
3. Inspect the domain mechanics for this pack: context pack, truncation risk, retrieval rule, summary boundary, freshness. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the context management plan, and list what should be routed to Agent Runtime Architecture Map, Eval Harness Spec, Runtime Failure Mode Review.
...
## Output
Return a context management plan with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which context pack signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready context management plan tied to context pack.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Agent Runtime Architecture Map, Eval Harness Spec, Runtime Failure Mode Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Context Window Management Plan workflow for the following Harness Engineering: The Runtime Layer Around AI Agents situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the context management plan using the output schema, mark assumptions, and list the human review gates before anyone acts.
