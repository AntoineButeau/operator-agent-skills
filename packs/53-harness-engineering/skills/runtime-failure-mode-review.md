# Runtime Failure Mode Review

## Use when
Use this workflow when you are working on Harness Engineering: The Runtime Layer Around AI Agents and need to diagnose the current state and name the operating risk. Its distinct job inside the Harness Engineering Pack is to produce the failure mode review; use the neighboring skills for Agent Runtime Architecture Map, Eval Harness Spec, Context Window Management Plan.
...
- Domain signals to inspect: timeout, tool error, hallucinated action, lost state, recovery workflow.
...
3. Inspect the domain mechanics for this pack: timeout, tool error, hallucinated action, lost state, recovery workflow. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the failure mode review, and list what should be routed to Agent Runtime Architecture Map, Eval Harness Spec, Context Window Management Plan.
...
## Output
Return a failure mode review with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which timeout signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready failure mode review tied to timeout.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Agent Runtime Architecture Map, Eval Harness Spec, Context Window Management Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Runtime Failure Mode Review workflow for the following Harness Engineering: The Runtime Layer Around AI Agents situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the failure mode review using the output schema, mark assumptions, and list the human review gates before anyone acts.
