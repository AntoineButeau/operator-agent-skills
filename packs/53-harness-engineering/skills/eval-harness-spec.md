...
## Use when
Use this workflow when you are working on Harness Engineering: The Runtime Layer Around AI Agents and need to handle the system step without overlapping the other skills in the pack. Its distinct job inside the Harness Engineering Pack is to produce the eval harness spec; use the neighboring skills for Agent Runtime Architecture Map, Context Window Management Plan, Runtime Failure Mode Review.
...
- Domain signals to inspect: test case, expected behavior, scoring rule, regression threshold, trace.
...
3. Inspect the domain mechanics for this pack: test case, expected behavior, scoring rule, regression threshold, trace. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the eval harness spec, and list what should be routed to Agent Runtime Architecture Map, Context Window Management Plan, Runtime Failure Mode Review.
...
## Output
Return a eval harness spec with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which test case signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready eval harness spec tied to test case.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Agent Runtime Architecture Map, Context Window Management Plan, Runtime Failure Mode Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Eval Harness Spec workflow for the following Harness Engineering: The Runtime Layer Around AI Agents situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the eval harness spec using the output schema, mark assumptions, and list the human review gates before anyone acts.
