...
## Use when
Use this workflow when you are working on Self-Regulation for Operators and need to turn the diagnosis into an executable plan with owners and gates. Its distinct job inside the Self-Regulation Operator Pack is to produce the response gap plan; use the neighboring skills for Trigger Pattern Map, Emotional Load Review, Operator Reset Protocol.
...
- Domain signals to inspect: pause tactic, reframe, boundary, next action, support.
...
3. Inspect the domain mechanics for this pack: pause tactic, reframe, boundary, next action, support. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the response gap plan, and list what should be routed to Trigger Pattern Map, Emotional Load Review, Operator Reset Protocol.
...
## Output
Return a response gap plan with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which pause tactic signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready response gap plan tied to pause tactic.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Trigger Pattern Map, Emotional Load Review, Operator Reset Protocol, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Response Gap Plan workflow for the following Self-Regulation for Operators situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the response gap plan using the output schema, mark assumptions, and list the human review gates before anyone acts.
