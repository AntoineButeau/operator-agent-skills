...
## Use when
Use this workflow when you are working on Self-Regulation for Operators and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the Self-Regulation Operator Pack is to produce the trigger pattern map; use the neighboring skills for Response Gap Plan, Emotional Load Review, Operator Reset Protocol.
...
- Domain signals to inspect: trigger, body cue, interpretation, impulse, consequence.
...
3. Inspect the domain mechanics for this pack: trigger, body cue, interpretation, impulse, consequence. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the trigger pattern map, and list what should be routed to Response Gap Plan, Emotional Load Review, Operator Reset Protocol.
...
## Output
Return a trigger pattern map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which trigger signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready trigger pattern map tied to trigger.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Response Gap Plan, Emotional Load Review, Operator Reset Protocol, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Trigger Pattern Map workflow for the following Self-Regulation for Operators situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the trigger pattern map using the output schema, mark assumptions, and list the human review gates before anyone acts.
