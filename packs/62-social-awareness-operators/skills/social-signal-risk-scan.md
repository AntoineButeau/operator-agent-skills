# Social Signal Risk Scan

## Use when
Use this workflow when you are working on Social Awareness for Operators and need to surface risks, constraints, and missing evidence before action. Its distinct job inside the Social Awareness Operator Pack is to produce the social signal risk scan; use the neighboring skills for Room Read Map, Stakeholder Empathy Brief, Meeting Dynamics Review.
...
- Domain signals to inspect: tone mismatch, power distance, context gap, audience sensitivity, trust level.
...
3. Inspect the domain mechanics for this pack: tone mismatch, power distance, context gap, audience sensitivity, trust level. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the social signal risk scan, and list what should be routed to Room Read Map, Stakeholder Empathy Brief, Meeting Dynamics Review.
...
## Output
Return a social signal risk scan with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which tone mismatch signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready social signal risk scan tied to tone mismatch.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Room Read Map, Stakeholder Empathy Brief, Meeting Dynamics Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Social Signal Risk Scan workflow for the following Social Awareness for Operators situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the social signal risk scan using the output schema, mark assumptions, and list the human review gates before anyone acts.
