...
## Use when
Use this workflow when you are working on decision making & systems thinking and need to handle the finder step without overlapping the other skills in the pack. Its distinct job inside the Judgment System Pack is to produce the constraint finder; use the neighboring skills for Decision Type Classifier, Second-Order Effects Scan, Incentive Behavior Review.
...
4. Separate this skill's job from the rest of the pack: deliver the constraint finder, and list what should be routed to Decision Type Classifier, Second-Order Effects Scan, Incentive Behavior Review.
...
## Output
Return a constraint finder with this schema:
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready constraint finder tied to one-way door.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Decision Type Classifier, Second-Order Effects Scan, Incentive Behavior Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Constraint Finder workflow for the following Decision Making & Systems Thinking situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the constraint finder using the output schema, mark assumptions, and list the human review gates before anyone acts.
