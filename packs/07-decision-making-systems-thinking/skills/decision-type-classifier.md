...
## Use when
Use this workflow when you are working on decision making & systems thinking and need to handle the classifier step without overlapping the other skills in the pack. Its distinct job inside the Judgment System Pack is to produce the decision type classifier; use the neighboring skills for Second-Order Effects Scan, Constraint Finder, Incentive Behavior Review.
...
- Domain signals to inspect: one-way door, constraint, incentive loop, second-order effect, decision log.
...
3. Inspect the domain mechanics for this pack: one-way door, constraint, incentive loop, second-order effect, decision log. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the decision type classifier, and list what should be routed to Second-Order Effects Scan, Constraint Finder, Incentive Behavior Review.
...
## Output
Return a decision type classifier with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which one-way door signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready decision type classifier tied to one-way door.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Second-Order Effects Scan, Constraint Finder, Incentive Behavior Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Decision Type Classifier workflow for the following Decision Making & Systems Thinking situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the decision type classifier using the output schema, mark assumptions, and list the human review gates before anyone acts.
