# Behavior Change Lever Map

## Use when
Use this workflow when you are working on how companies actually behave: operating principles, decision rules, and the hidden system behind culture and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the Behavioral OS Pack is to produce the owner-and-flow map; use the neighboring skills for Revealed Behavior Audit, Decision Rule Extractor, Culture Operating Gap Review.
...
4. Separate this skill's job from the rest of the pack: deliver the owner-and-flow map, and list what should be routed to Revealed Behavior Audit, Decision Rule Extractor, Culture Operating Gap Review.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready owner-and-flow map tied to revealed behavior.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Revealed Behavior Audit, Decision Rule Extractor, Culture Operating Gap Review, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Behavior Change Lever Map workflow for the following How Companies Actually Behave: Operating Principles, Decision Rules, and the Hidden System Behind Culture situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the owner-and-flow map using the output schema, mark assumptions, and list the human review gates before anyone acts.
