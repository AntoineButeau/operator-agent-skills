...
## Use when
Use this workflow when you are working on how companies actually behave: operating principles, decision rules, and the hidden system behind culture and need to handle the extractor step without overlapping the other skills in the pack. Its distinct job inside the Behavioral OS Pack is to produce the decision rule extractor; use the neighboring skills for Revealed Behavior Audit, Culture Operating Gap Review, Behavior Change Lever Map.
...
- Domain signals to inspect: revealed behavior, decision rule, culture gap, behavior lever, operating norm.
...
3. Inspect the domain mechanics for this pack: revealed behavior, decision rule, culture gap, behavior lever, operating norm. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the decision rule extractor, and list what should be routed to Revealed Behavior Audit, Culture Operating Gap Review, Behavior Change Lever Map.
...
## Output
Return a decision rule extractor with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which revealed behavior signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready decision rule extractor tied to revealed behavior.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Revealed Behavior Audit, Culture Operating Gap Review, Behavior Change Lever Map, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Decision Rule Extractor workflow for the following How Companies Actually Behave: Operating Principles, Decision Rules, and the Hidden System Behind Culture situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the decision rule extractor using the output schema, mark assumptions, and list the human review gates before anyone acts.
