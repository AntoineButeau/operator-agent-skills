# Meeting System Prune

## Use when
Use this workflow when you are working on operating cadence & management systems and need to handle the prune step without overlapping the other skills in the pack. Its distinct job inside the Operating Cadence Pack is to produce the meeting system prune; use the neighboring skills for Cadence Architecture Map, Weekly Business Review Design, Cadence Health Check.
...
4. Separate this skill's job from the rest of the pack: deliver the meeting system prune, and list what should be routed to Cadence Architecture Map, Weekly Business Review Design, Cadence Health Check.
...
## Output
Return a meeting system prune with this schema:
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready meeting system prune tied to WBR.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Cadence Architecture Map, Weekly Business Review Design, Cadence Health Check, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Meeting System Prune workflow for the following Operating Cadence & Management Systems situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the meeting system prune using the output schema, mark assumptions, and list the human review gates before anyone acts.
