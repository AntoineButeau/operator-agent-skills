...
## Use when
Use this workflow when you are working on Workflow Ontology and need to find gaps between stated process and observed behavior. Its distinct job inside the Workflow Ontology Pack is to produce the state transition audit; use the neighboring skills for Workflow Entity Map, Work Object Taxonomy, Ontology Change Control.
...
- Domain signals to inspect: workflow state, handoff condition, invalid state, trigger, completion rule.
...
3. Inspect the domain mechanics for this pack: workflow state, handoff condition, invalid state, trigger, completion rule. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the state transition audit, and list what should be routed to Workflow Entity Map, Work Object Taxonomy, Ontology Change Control.
...
## Output
Return a state transition audit with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which workflow state signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready state transition audit tied to workflow state.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Workflow Entity Map, Work Object Taxonomy, Ontology Change Control, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the State Transition Audit workflow for the following Workflow Ontology situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the state transition audit using the output schema, mark assumptions, and list the human review gates before anyone acts.
