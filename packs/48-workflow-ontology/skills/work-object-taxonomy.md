...
- Domain signals to inspect: request, task, project, case, decision, record.
...
3. Inspect the domain mechanics for this pack: request, task, project, case, decision, record. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the work object taxonomy, and list what should be routed to Workflow Entity Map, State Transition Audit, Ontology Change Control.
...
## Output
Return a work object taxonomy with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which request signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready work object taxonomy tied to request.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Workflow Entity Map, State Transition Audit, Ontology Change Control, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Work Object Taxonomy workflow for the following Workflow Ontology situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the work object taxonomy using the output schema, mark assumptions, and list the human review gates before anyone acts.
