# Escalation Judgment Check

## Use when
Use this workflow when you are working on managing up and need to diagnose the current state and name the operating risk. Its distinct job inside the Upward Signal Pack is to produce the escalation judgment check; use the neighboring skills for Manager Trust Update, Upward Disagreement Brief, Priority Reading Map.
...
- Domain signals to inspect: manager trust ledger, priority stack, escalation threshold, disagreement memo, decision owner.
...
3. Inspect the domain mechanics for this pack: manager trust ledger, priority stack, escalation threshold, disagreement memo, decision owner. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the escalation judgment check, and list what should be routed to Manager Trust Update, Upward Disagreement Brief, Priority Reading Map.
...
## Output
Return a escalation judgment check with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which manager trust ledger signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready escalation judgment check tied to manager trust ledger.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Manager Trust Update, Upward Disagreement Brief, Priority Reading Map, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Escalation Judgment Check workflow for the following Managing Up situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the escalation judgment check using the output schema, mark assumptions, and list the human review gates before anyone acts.
