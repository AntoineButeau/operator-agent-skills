# Manager Trust Update

## Use when
Use this workflow when you are working on managing up and need to compress the situation into a decision-ready brief. Its distinct job inside the Upward Signal Pack is to produce the operator memo; use the neighboring skills for Upward Disagreement Brief, Escalation Judgment Check, Priority Reading Map.
...
4. Separate this skill's job from the rest of the pack: deliver the operator memo, and list what should be routed to Upward Disagreement Brief, Escalation Judgment Check, Priority Reading Map.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready operator memo tied to manager trust ledger.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Upward Disagreement Brief, Escalation Judgment Check, Priority Reading Map, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Manager Trust Update workflow for the following Managing Up situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the operator memo using the output schema, mark assumptions, and list the human review gates before anyone acts.
