...
## Use when
Use this workflow when you are working on AI Quality Systems and need to handle the system step without overlapping the other skills in the pack. Its distinct job inside the AI Quality Systems Pack is to produce the eval suite; use the neighboring skills for AI Quality Bar Definition, Model Behavior Drift Review, Quality Incident Response Plan.
...
- Domain signals to inspect: golden case, edge case, regression, judge prompt, score threshold.
...
3. Inspect the domain mechanics for this pack: golden case, edge case, regression, judge prompt, score threshold. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the eval suite, and list what should be routed to AI Quality Bar Definition, Model Behavior Drift Review, Quality Incident Response Plan.
...
## Output
Return a eval suite with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which golden case signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready eval suite tied to golden case.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with AI Quality Bar Definition, Model Behavior Drift Review, Quality Incident Response Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Eval Suite Builder workflow for the following AI Quality Systems situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the eval suite using the output schema, mark assumptions, and list the human review gates before anyone acts.
