# Model Behavior Drift Review

## Use when
Use this workflow when you are working on AI Quality Systems and need to diagnose the current state and name the operating risk. Its distinct job inside the AI Quality Systems Pack is to produce the drift review; use the neighboring skills for AI Quality Bar Definition, Eval Suite Builder, Quality Incident Response Plan.
...
- Domain signals to inspect: behavior change, prompt dependency, model version, output variance, monitoring signal.
...
3. Inspect the domain mechanics for this pack: behavior change, prompt dependency, model version, output variance, monitoring signal. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the drift review, and list what should be routed to AI Quality Bar Definition, Eval Suite Builder, Quality Incident Response Plan.
...
## Output
Return a drift review with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which behavior change signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready drift review tied to behavior change.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with AI Quality Bar Definition, Eval Suite Builder, Quality Incident Response Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Model Behavior Drift Review workflow for the following AI Quality Systems situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the drift review using the output schema, mark assumptions, and list the human review gates before anyone acts.
