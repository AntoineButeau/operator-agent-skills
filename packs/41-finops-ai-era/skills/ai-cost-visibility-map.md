# AI Cost Visibility Map

## Use when
Use this workflow when you are working on FinOps for the AI Era and need to make the system visible by mapping owners, flows, and evidence. Its distinct job inside the AI-Era FinOps Pack is to produce the cost visibility map; use the neighboring skills for Model Spend Governance Brief, Unit Economics Scenario Review, Optimization Backlog Builder.
...
- Domain signals to inspect: model spend, unit economics, usage driver, allocation rule, owner behavior.
...
3. Inspect the domain mechanics for this pack: model spend, unit economics, usage driver, allocation rule, owner behavior. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the cost visibility map, and list what should be routed to Model Spend Governance Brief, Unit Economics Scenario Review, Optimization Backlog Builder.
...
## Output
Return a cost visibility map with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which model spend signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready cost visibility map tied to model spend.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Model Spend Governance Brief, Unit Economics Scenario Review, Optimization Backlog Builder, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the AI Cost Visibility Map workflow for the following FinOps for the AI Era situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the cost visibility map using the output schema, mark assumptions, and list the human review gates before anyone acts.
