# Unit Economics Scenario Review

## Use when
Use this workflow when you are working on FinOps for the AI Era and need to diagnose the current state and name the operating risk. Its distinct job inside the AI-Era FinOps Pack is to produce the scenario review; use the neighboring skills for AI Cost Visibility Map, Model Spend Governance Brief, Optimization Backlog Builder.
...
- Domain signals to inspect: inference cost, margin exposure, customer tier, pricing assumption, utilization curve.
...
3. Inspect the domain mechanics for this pack: inference cost, margin exposure, customer tier, pricing assumption, utilization curve. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the scenario review, and list what should be routed to AI Cost Visibility Map, Model Spend Governance Brief, Optimization Backlog Builder.
...
## Output
Return a scenario review with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which inference cost signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready scenario review tied to inference cost.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with AI Cost Visibility Map, Model Spend Governance Brief, Optimization Backlog Builder, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Unit Economics Scenario Review workflow for the following FinOps for the AI Era situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the scenario review using the output schema, mark assumptions, and list the human review gates before anyone acts.
