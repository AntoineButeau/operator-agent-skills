...
## Use when
Use this workflow when you are working on Agent-Native Tools and need to diagnose the current state and name the operating risk. Its distinct job inside the Agent-Native Tools Pack is to produce the agent UX review; use the neighboring skills for Agent Tool Fit Check, Tool Context Contract, Tool Evaluation Plan.
...
- Domain signals to inspect: tool discoverability, action feedback, ambiguity handling, confirmation step, recovery path.
...
3. Inspect the domain mechanics for this pack: tool discoverability, action feedback, ambiguity handling, confirmation step, recovery path. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the agent UX review, and list what should be routed to Agent Tool Fit Check, Tool Context Contract, Tool Evaluation Plan.
...
## Output
Return a agent UX review with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which tool discoverability signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready agent UX review tied to tool discoverability.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Agent Tool Fit Check, Tool Context Contract, Tool Evaluation Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Agent UX Review workflow for the following Agent-Native Tools situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the agent UX review using the output schema, mark assumptions, and list the human review gates before anyone acts.
