# Agent Tool Fit Check

## Use when
Use this workflow when you are working on Agent-Native Tools and need to handle the system step without overlapping the other skills in the pack. Its distinct job inside the Agent-Native Tools Pack is to produce the tool fit check; use the neighboring skills for Tool Context Contract, Agent UX Review, Tool Evaluation Plan.
...
- Domain signals to inspect: agent affordance, API surface, task context, human override, observability.
...
3. Inspect the domain mechanics for this pack: agent affordance, API surface, task context, human override, observability. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the tool fit check, and list what should be routed to Tool Context Contract, Agent UX Review, Tool Evaluation Plan.
...
## Output
Return a tool fit check with this schema:
- `use_case`: the specific operating situation and decision.
- `evidence`: facts, source links, missing data, and assumption flags.
- `diagnosis`: what is happening, why it matters, and which agent affordance signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready tool fit check tied to agent affordance.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Tool Context Contract, Agent UX Review, Tool Evaluation Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Agent Tool Fit Check workflow for the following Agent-Native Tools situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the tool fit check using the output schema, mark assumptions, and list the human review gates before anyone acts.
