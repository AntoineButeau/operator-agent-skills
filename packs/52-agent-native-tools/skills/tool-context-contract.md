...
## Use when
Use this workflow when you are working on Agent-Native Tools and need to handle the system step without overlapping the other skills in the pack. Its distinct job inside the Agent-Native Tools Pack is to produce the context contract; use the neighboring skills for Agent Tool Fit Check, Agent UX Review, Tool Evaluation Plan.
...
- Domain signals to inspect: input schema, state requirement, permission, expected output, error behavior.
...
3. Inspect the domain mechanics for this pack: input schema, state requirement, permission, expected output, error behavior. Call out where handoffs, incentives, review gates, or decision rights break down.
4. Separate this skill's job from the rest of the pack: deliver the context contract, and list what should be routed to Agent Tool Fit Check, Agent UX Review, Tool Evaluation Plan.
...
- `diagnosis`: what is happening, why it matters, and which input schema signal changed.
...
## Failure modes
- Producing a generic consultant summary instead of a decision-ready context contract tied to input schema.
- Treating stale dashboards, anecdotes, or meeting memory as facts without source-of-truth checks.
- Blurring this skill with Agent Tool Fit Check, Agent UX Review, Tool Evaluation Plan, which creates duplicate work and hides the real owner.
- Skipping the human review gate for commitments, sensitive people/customer data, or system-of-record changes.

## Example prompt
Run the Tool Context Contract workflow for the following Agent-Native Tools situation. Context: <describe team, decision, deadline, evidence links, owner, and known constraints>. Return the context contract using the output schema, mark assumptions, and list the human review gates before anyone acts.
